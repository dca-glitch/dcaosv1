...
- **AI Knowledge Base / Context Builder**: admin-only, route-hard-gated (`requireRole("owner","admin")`) reusable memory layer (`AiKnowledgeItem`, versioned, scoped SYSTEM/CLIENT/PROJECT/INDUSTRY) and context assembly service (`ai-context-builder.service.ts`) that composes approved (`status=APPROVED`, `allowedForPrompt=true`, non-expired) knowledge into AI Delivery workflow run execution context, with prompt-injection sanitization and immutable `AiContextSnapshot` audit records. See [`docs/modules/KNOWLEDGE_BASE.md`](./modules/KNOWLEDGE_BASE.md) and [`docs/ai-delivery/ai-operating-layer-architecture.md`](./ai-delivery/ai-operating-layer-architecture.md). **Confirmed gap (Block 5A):** this layer currently feeds only AiDelivery workflow-run execution — WorkflowBriefs' own AI-run pipeline (`triggerWorkflowBriefAiRun` and related functions) does not query it at all; the two composable-context mechanisms are disconnected today (not a bug, but a real architecture gap — see `WORKFLOW_BRIEFS_MODULE_PLAN.md`).
