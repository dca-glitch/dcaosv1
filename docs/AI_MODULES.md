...
- **AI Knowledge Base / Context Builder**: admin-only, route-hard-gated (`requireRole("owner","admin")`) reusable memory layer (`AiKnowledgeItem`, versioned, scoped SYSTEM/CLIENT/PROJECT/INDUSTRY) and context assembly service (`ai-context-builder.service.ts`) that composes approved (`status=APPROVED`, `allowedForPrompt=true`, non-expired) knowledge into **AiDelivery workflow-run execution** and **WorkflowBriefs MI/SEO AI-run** context (Block 6A foundation), with prompt-injection sanitization and immutable `AiContextSnapshot` audit records. See [`docs/modules/KNOWLEDGE_BASE.md`](./modules/KNOWLEDGE_BASE.md) and [`docs/ai-delivery/ai-operating-layer-architecture.md`](./ai-delivery/ai-operating-layer-architecture.md). Plan/draft generation paths remain unwired — see `WORKFLOW_BRIEFS_MODULE_PLAN.md`.
