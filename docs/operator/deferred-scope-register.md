...
**Canonical component system (2026-07-13):** Authoritative decision + Wave 1 first consolidation slice recorded in [`CANONICAL_COMPONENT_SYSTEM.md`](../project-control/CANONICAL_COMPONENT_SYSTEM.md). Public API = `apps/web/src/components/ui`; private foundation = `apps/web/src/design-system`. Import guard baseline shrunk to **24** (Modal + bridge only). Unused root Empty/Error/Loading/StatusNotice shims deleted. Modal Wave remains deferred / separately gated. Production remains frozen.
