...
**WORDPRESS DEDICATED LIVE-DRAFT ADAPTER (2026-07-12):** Local KEEP — `createWordPressDraft` + exact-ID trash; username on `PublicationTarget`; Application Password remains encrypted; dual `WORDPRESS_DRAFT_LIVE_*` flags default false; generic publish remains `WORDPRESS_LIVE_HTTP_FROZEN`. Fake-transport smoke PASS. Staging live draft NOT PROVEN. Production FROZEN. Next: owner-approved staging one-draft live proof.
