...
**Modal Wave (2026-07-13):** Canonical Modal = `apps/web/src/components/ui/Modal` (design-system adapter + portal). All **23** former `components/Modal` page consumers migrated to `components/ui`. Legacy root `components/Modal.tsx` **deleted**. Import-guard baseline **0**. Commit `a447b9e`. Staging web-only deploy COMPLETE at artifact `/opt/dca/staging-artifacts/a447b9e` (local web dist + Caddy remount); staging Modal browser proof PASS; API/DB identities unchanged; live flags false; production API/DB unchanged. `aria-describedby` / nested-dialog stack / `alertdialog` remain documented exceptions (no DS edit). Production remains frozen.
