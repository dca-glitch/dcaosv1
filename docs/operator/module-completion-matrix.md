...
| AI Knowledge / Context layer | Partial — execution wired (Blocks 5A/6A/6B); admin visibility implemented (6C-v1) | Admin-only, route-hard-gated reusable memory (`AiKnowledgeItem`/`AiContextSnapshot`) feeding AiDelivery workflow-run and WorkflowBriefs MI/SEO/plan/draft execution context (Blocks 5A/6A/6B); confirmed no client-reachable exposure. Block 6C-v1 adds admin read-only `knowledgeContext` visibility on Workflow Briefs screens. Knowledge picker/override (6C-v2) and `AiContextSnapshot` per-brief audit rows (6D) remain deferred. AI SEO manual CRUD and PDF export are intentionally not Knowledge execution paths; workflow-generated content plans already use the AiDelivery workflow Knowledge path — see [`docs/modules/SEO_MODULE_PLAN.md`](../modules/SEO_MODULE_PLAN.md). |
