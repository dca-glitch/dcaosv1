...
| AI Knowledge / Context layer | Closed for admin-only MVP (Blocks 5A/6A/6B) | Admin-only, route-hard-gated reusable memory (`AiKnowledgeItem`/`AiContextSnapshot`) feeding AiDelivery workflow-run and WorkflowBriefs MI/SEO/plan/draft context (Blocks 6A/6B); confirmed no client-reachable exposure. Separate AI SEO module content-plan/PDF path remains unwired — see [`docs/modules/SEO_MODULE_PLAN.md`](../modules/SEO_MODULE_PLAN.md). |
