...
| AI Knowledge / Context layer | Partial — execution wired (Blocks 5A/6A/6B); admin visibility pending (6C-v1) | Admin-only, route-hard-gated reusable memory (`AiKnowledgeItem`/`AiContextSnapshot`) feeding AiDelivery workflow-run and WorkflowBriefs MI/SEO/plan/draft execution context (Blocks 5A/6A/6B); confirmed no client-reachable exposure. Admin read-only `knowledgeContext` visibility on Workflow Briefs screens is planned as Block 6C-v1. Separate AI SEO module content-plan/PDF path remains unwired — see [`docs/modules/SEO_MODULE_PLAN.md`](../modules/SEO_MODULE_PLAN.md). |
