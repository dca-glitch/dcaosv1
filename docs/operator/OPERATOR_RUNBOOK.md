...
**Status:** Single operator entry point for local validation, smoke, recovery, and staging/production prerequisites. Current production state: clean-state reset PASS, interactive admin login verified, Turnstile temporarily disabled, and production healthy; future mutations remain owner-approved only. Live proofs are still separate gates, and no smoke runs after failed validate. PowerShell one-command-per-line.
...
| Production clean-state reset record | [`docs/operator/PRODUCTION_CLEAN_STATE_RESET_2026-07-13.md`](./PRODUCTION_CLEAN_STATE_RESET_2026-07-13.md) |
