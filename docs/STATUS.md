...
- **Admin Operations Block 2 (read-only):** `GET /api/v1/admin/operations/summary`, dashboard Operational readiness panel, delivery audit filter, recovery hints runbook [`docs/runbooks/ADMIN_OPERATIONS_RECOVERY.md`](./runbooks/ADMIN_OPERATIONS_RECOVERY.md), and `npm run smoke:admin-operations:local` for admin/client diagnostics boundary. No durable closeout store; manual-run guidance only. Staging/production untouched.
- **UI/UX Block 3 (frontend-only consolidation):** darker near-black canvas, compact admin shell density, dimmed design-system primary actions, quieter status/guidance blocks, and surface-specific polish for Dashboard, Clients/Projects shell, AI Delivery, Market Intelligence, Monthly Reports, Client Portal, and Finance Lite tables — CSS-only in `apps/web/src/styles.css`; no backend/schema/API/auth behavior changes. Staging/production untouched.
