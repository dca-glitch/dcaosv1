# DCA OS Lite â€” Status (Source of Truth)

**Last updated:** 2026-07-14 (G49 production GO/NO-GO review = `CONDITIONAL_GO` for candidate `c5e03eb`; production unchanged; staging browser closeout on `main` `@01b7e04`)
**Production runtime:** `57f9c524118a5bf6c93270626cef7c8bd52d140b` / image `bd61d5deb331` Â· **local/main tip:** `01b7e04ef7701acd7e6bfe72e00721f2c8c771b1` Â· **staging-proven runtime (G50 candidate):** `c5e03eb52f882a5d6f9e8f92fc1794a91f3289c5` (**not** production-redeployed; production deploy **not** authorized)
...
**Production classification:** `CLEAN_FIRST_RUN_OWNER_SETUP` Â· `TURNSTILE_ENABLED=false` (temporary) Â· `PURIVA_FULL_LAUNCH=NOT_YET_CLAIMED` Â· live provider flags false Â· `SHARED_PROXY_ACTION=none`
...
> **LIVE GA4/GSC WITHDRAWN (2026-07-13, owner decision):** Live Google Analytics 4 and Google Search Console integrations are **withdrawn** from current and planned scope â€” not deferred for automatic resumption. No live OAuth, service accounts, sync, property discovery, refresh tokens, or scheduled sync. Manual CSV import is **not** implemented and **not** authorized now. Generic monthly-report / MANUAL / CSV_IMPORT metric structures remain. Schema fields `gscSiteUrl` / `ga4PropertyId` and enum values GA4/GSC/HYBRID remain for `SCHEMA_DEPRECATE_LATER`. **Google Drive** remains a separate capability. Live GA4/GSC is **not** a Puriva Launch blocker. Historical paragraphs below that still say "deferred" or "next gate = GA/GSC" are superseded by this closeout.

> **LOCAL API/AUTH DAST (2026-07-13):** Earlier API/auth dynamic testing was blocked by unprepared local PostgreSQL (`127.0.0.1:5434`). That blocker is resolved. Local Postgres + API health ready; `smoke:local`, `smoke:mvp:local`, client-role boundary, tenant-module, and API integration auth tests passed. Confirmed repairs: malformed JSON â†’ `400 INVALID_JSON` without stack/path leakage; password change/admin reset revoke all active sessions. Staging proof **not required** for this local-first hardening. Production was not tested or mutated.
...
**G57â€“G68 pre-live readiness:** [`docs/runbooks/G57_G68_PRELIVE_READINESS.md`](./runbooks/G57_G68_PRELIVE_READINESS.md)
**G70 live proof checklist:** [`docs/runbooks/AI_PROVIDER_LIVE_PROOF.md`](./runbooks/AI_PROVIDER_LIVE_PROOF.md) Â§9
**G72 model routing policy:** [`docs/runbooks/AI_MODEL_ROUTING_POLICY.md`](./runbooks/AI_MODEL_ROUTING_POLICY.md)
**AI policy:** [`docs/ai/AI_MODEL_POLICY.md`](./ai/AI_MODEL_POLICY.md) Â· [`docs/architecture/AI_MODEL_POLICY.md`](./architecture/AI_MODEL_POLICY.md) Â· [`docs/architecture/AI_POLICY_PROVIDER_ROUTING.md`](./architecture/AI_POLICY_PROVIDER_ROUTING.md) Â· [`docs/ai/AI_ORCHESTRATOR_LITE.md`](./ai/AI_ORCHESTRATOR_LITE.md)
**Operator index:** [`docs/operator/OPERATOR_RUNBOOK.md`](./operator/OPERATOR_RUNBOOK.md)
**Architecture map:** [`docs/ARCHITECTURE.md`](./ARCHITECTURE.md) Â§ Current application map
...
**Live proof runbooks:** [`AI_PROVIDER_LIVE_PROOF.md`](./runbooks/AI_PROVIDER_LIVE_PROOF.md) Â· [`WORDPRESS_DRAFT_PROOF.md`](./runbooks/WORDPRESS_DRAFT_PROOF.md) Â· [`MONTHLY_REPORT_LIVE_DATA_PROOF.md`](./runbooks/MONTHLY_REPORT_LIVE_DATA_PROOF.md)
...
**Production deployment/rollback/checklist (reference):** [`docs/runbooks/PRODUCTION_DEPLOYMENT.md`](./runbooks/PRODUCTION_DEPLOYMENT.md) Â· [`docs/runbooks/PRODUCTION_ROLLBACK.md`](./runbooks/PRODUCTION_ROLLBACK.md) Â· [`docs/runbooks/PRODUCTION_SAFETY_CHECKLIST.md`](./runbooks/PRODUCTION_SAFETY_CHECKLIST.md)
...
**Admin UI/workflow polish audit:** [`docs/ux/ADMIN_WORKFLOW_POLISH_AUDIT.md`](./ux/ADMIN_WORKFLOW_POLISH_AUDIT.md) (read-only; UX-P1â€“P12 queued blocks)
...
## Executive snapshot (G52-B + G53 + G55 + G56 + G57â€“G68 + G69 + G89-G708 + PRE-STAGING)
...
| **PRE-STAGING local closure** | **PASS (local/no-live only)** â€” see [`PRE_STAGING_CLOSURE_VERDICT.md`](./operator/PRE_STAGING_CLOSURE_VERDICT.md). Historical; superseded by production deploy for env readiness |
| **G69 merge** | **DONE** â€” G57â€“G68 fast-forward merged to `main`; final commit `64bfd06` |
| Production readiness | **CONTROLLED LIVE TESTING READY** â€” runtime `57f9c52` deployed; external providers owner-gated; Puriva full launch NOT claimed |
| Next gate | **G50 production deploy execution â€” owner approval required** naming exact candidate `c5e03eb`. G49 dry-run 2026-07-14 = `CONDITIONAL_GO` (fresh pre-G50 backups + explicit migration SKIP). Production remains on `57f9c52` until G50. Puriva Launch remains separate. **Live GA4/GSC: WITHDRAWN â€” not next gate.** Evidence: [`docs/audits/PRODUCTION_DEPLOY_GO_NO_GO_REVIEW_2026-07-14.md`](./audits/PRODUCTION_DEPLOY_GO_NO_GO_REVIEW_2026-07-14.md). |
| PRE-STAGING Lanes 14â€“15 | **Docs closeout** â€” stale-claim sweep + operator runbook refresh; no live proof; Puriva Launch **BLOCKED** |
| G469-G708 final integration | **KEEP** â€” 20 lanes reconciled; local/no-IO foundations only; live R2, live email, live WordPress, live image, staging/prod live proofs, full notification E2E launch proof, trusted `actualCostUsd` ingestion, commit, push, deploy remain blocked (in-app notification persistence/UI foundation now exists â€” see notification rows below). **Live GA4/GSC: WITHDRAWN.** |
| G229-G468 final integration | **KEEP** â€” 12 lanes reconciled; local/no-IO foundations only; superseded as latest baseline by G469-G708 |
| G72 model routing | **Implemented** â€” backend policy per task type; approved model `anthropic/claude-haiku-4.5`; no live call in G72 |
| G73 routing attribution | **Local PASS** â€” dry-run/preview `modelRouting` + `plannedLedgerMetadata`; budget guard route cap wired; persistent preview ledger records routing metadata |
| G74 completed ledger attribution | **Implemented (no-live)** â€” `buildCompletedLedgerMetadata`, `prepareCompletedLedgerAttribution`, `recordCompletedAiLedgerEntry`; mocked provider execution in unit tests |
| G75 live spend attribution proof | **PARTIAL (local only)** â€” live OpenRouter smoke PASS; completed attribution verifier PASS (G75c); generated metadata only at G75 time â€” wiring gap since closed in G76 |
| G76 persistent completed ledger wiring | **Implemented (mocked/no-live)** â€” AI Delivery execute success path persists COMPLETED row; live DB row since proven in G77b |
| G77 persistent COMPLETED ledger live proof | **COMPLETE (local only)** â€” G77b live smoke + ledger verifier PASS; row `5d8d635c-ced0-4a14-9b33-839e1fdee508` for run `2244413e-d87b-45a1-8a26-6634ec8972d5` |
| G77b closeout | **COMPLETE (local only)** â€” local controlled live proof recorded; production remains frozen; staging/production live not claimed |
| G78 multi-scope docs | **Docs only â€” complete (uncommitted)** â€” runbook truth labels, deferred-scope buckets, notifications plan, integration/monthly-report/WordPress tier labels, pack implementation status |
| G79 monthly cap aggregation | **Implemented (local unit proof)** â€” `sumSpentUsdForPeriod()` includes live `COMPLETED` rows and uses `actualCostUsd ?? estimatedCostUsd`; `ai-budget-ledger.service.test.ts` 18/18 PASS |
| G80 `actualCostUsd` policy | **Policy documented only** â€” `actualCostUsd` remains `null` unless a trusted provider cost is exposed; no invoice-cost ingestion implemented |
| G81-G87 live proof planning | **Planning/prep only** â€” staging live proof, notifications, email no-send proof, approval event map, WordPress live draft, and image generation provider proof plans updated; no live calls or sends executed. **GA/GSC live metrics planning superseded â€” live scope WITHDRAWN.** |
| G88 consolidation | **Docs consolidation in shared files** â€” correct G79-G88 gate map recorded; Puriva Launch remains **BLOCKED** and production remains frozen |
| G89-G148 final integration | **KEEP** â€” local foundations only; live proofs remain blocked |
| G149-G228 final integration | **KEEP** â€” 9 lanes reconciled; local/no-IO foundations only; superseded as latest baseline by G469-G708 |
| Notification persistence / inbox | **LOCAL FOUNDATION** â€” Prisma `InAppNotification`, migration `20260711115000_add_in_app_notifications` (applied on staging with `1b8d00d`), service/controller/API, integration tests, frontend `NotificationPanel`; full E2E/launch proof **not** closed â€” see [`AUTHORITATIVE_PROJECT_CONTROL_MATRIX.md`](./project-control/AUTHORITATIVE_PROJECT_CONTROL_MATRIX.md) |
| Live email / Resend | **STAGING PROVIDER ACCEPTANCE PROVEN** â€” one owner-controlled adapter-only send on staging `a8a74e6` (`AI_DELIVERY_APPROVED`); EmailLog SENT + provider message id; **not** inbox/webhook delivery; `EMAIL_LIVE_SEND_AUTHORIZED=false` / `sendingEnabled=false` restored |
| Dependency audit (2026-07-13 re-verify) | **Vite high RESOLVED** â€” `GHSA-fx2h-pf6j-xcff` / Vite `6.4.3` (remediation `95af080`); re-check at `622b1ed`: `npm ls` single copy, `npm audit` high=0 vite-related=0; moderate uuid/googleapis (4) remain **not** claimed closed |
| Trusted `actualCostUsd` ingestion | **DEFERRED** â€” G80 policy + G389-G408 design helpers; ingestion not wired |
| G134-G137 AI budget reporting contract | **Implemented locally (contract + unit proof)** â€” additive reporting/reconciliation contract separates AI budget from Finance Lite, reports monthly cap/live rows/estimated-vs-actual/provider/model, and keeps invoice reconciliation `not_integrated`; no real invoice or Finance Lite mutation |
| G76c review | **KEEP** â€” PowerShell review-only; no code changes |
| G76d closeout | **Docs only** â€” wiring + limitations recorded; production remains frozen |
| G75c verifier fix | **TEMP only** â€” verifier/tooling fix (`node --import tsx` + cross-client live evidence scan); repo source unchanged; no new live call |
| G75d closeout | **Docs only** â€” local proof + limitation recorded; production remains frozen |
| G55 pre-live | **Local PASS** â€” docs + disabled-safe orchestration foundation; no live providers/deploy |
| G56 pre-live | **Local PASS** â€” expanded pre-live groundwork; admin orchestrator UI; orchestrator smoke; no live providers/deploy |
| G57â€“G68 pre-live | **On main** (`64bfd06`) â€” persistent ledger, dry-run adapter, operator visibility, go/no-go docs; **local live AI + COMPLETED ledger row proven (G77b)**; staging/production live proofs **BLOCKED** |
| G70 closeout | **Docs only** â€” post-G69 STATUS/deferred/live-proof checklist; no live execution |
| G71b live proof retry | **Procedural STOP** â€” one safe live OpenRouter call captured; baseline smoke failed because API was already on `openrouter` live; see [`AI_PROVIDER_LIVE_PROOF.md`](./runbooks/AI_PROVIDER_LIVE_PROOF.md) Â§9.13 |
| G71c closeout | **Docs only** â€” partial live proof recorded; local gateway restored; production remains frozen |
| G71e / G71e-retry | **Formal clean local live proof COMPLETE** â€” Phase 1 baseline PASS; Phase 2 live PASS (`90941e76-260d-4f99-b299-3a5c6b7a8d65`); restore PASS; model `anthropic/claude-haiku-4.5` |
| G71f closeout | **Docs only** â€” formal clean proof recorded; production remains frozen |
| Staging | **Proven** â€” controlled staging deploy of `1b8d00d` completed; post-deploy smoke PASS; migrations `20260709120000_add_ai_budget_ledger` and `20260711115000_add_in_app_notifications` applied; prior G46d/G47 PASS on `5e1ea5a` is historical context only |
| Production deploy | **Frozen/deferred** â€” no deploy until G49 dry-run + G50 explicit approval |
| G49 public probes (Â§6.2) | **PASS** â€” historical 2026-07-09; **formal G49 dry-run PASS** 2026-07-14 (`CONDITIONAL_GO` â†’ G50 owner approval for `c5e03eb`; production deploy still **not** authorized) |
| G50 production deploy | **Not executed** â€” frozen/deferred |
| Puriva Launch | **Blocked** â€” live proof gates and product workflow gates required; G89-G707 add local foundations and docs only; no live provider/storage/email/Google/WordPress/image proof, staging/prod proof, or production deploy is claimed |
| Roadmap tracks | Production Safety Â· Live Integration Proof Â· Client Operating Pack/Productization |
| Docker non-root runtime | **STAGING NON-ROOT PROVEN** â€” exact commit `80569c68f94481b33dd0a3c2a5a3ec17b41e31cd`; local candidate image `Config.User=node`; local runtime UID/GID `1000/1000`; staging runtime UID/GID `1000/1000`; no permission or restart regressions; production non-root proof remains unproven and requires separate owner-approved production deploy/proof |

Detail: [`AI_MODEL_ROUTING_POLICY.md`](./runbooks/AI_MODEL_ROUTING_POLICY.md) Â· [`G53_PRODUCTION_SAFETY_PLAN.md`](./runbooks/G53_PRODUCTION_SAFETY_PLAN.md) Â· [`deferred-scope-register.md`](./operator/deferred-scope-register.md) Â· Â§2.14 below.
...
| `dcaosv1-postgres` | Unchanged â€” `StartedAt` 2026-06-14T10:52:09Z, Restarts=0 |
| `dca-caddy` | Unchanged â€” `StartedAt` 2026-07-09T00:59:48Z, Restarts=0 |
| `dcaosv1-staging-api` | Unchanged â€” `StartedAt` 2026-07-11T10:01:34Z, Restarts=0 |
| `dcaosv1-staging-postgres` | Unchanged â€” `StartedAt` 2026-07-04T00:38:06Z, Restarts=0 |
...
| Production general deploy approval | **Not granted** â€” production remains frozen for unrelated deployment |
...
**Future gates:**
- **Phase B â€” Turnstile cutover:** requires new owner-supplied Cloudflare Turnstile secret.
- **Phase C â€” R2 cutover:** requires new owner-supplied Cloudflare R2 access key pair.
- **Phase D â€” old external credential revocation:** requires literal token `APPROVE_EXTERNAL_CREDENTIAL_REVOCATION`.
...
**Result:** PASS â€” controlled staging deployment of commit `a8a74e6` completed; health PASS; required staging smokes PASS; no migration performed; production untouched.
...
| Local mega smoke note | `smoke:staging-readiness:local` hung/skipped â€” does not invalidate staging PASS |
...
| `prisma migrate deploy` | **not performed** â€” read-only status: Database schema is up to date (47 migrations) |
...
| Staging API container | `1c768ba94d0eâ€¦`; StartedAt `2026-07-12T04:31:06Z`; `127.0.0.1:4011->4000` |
...
| DB container | `c70f523dâ€¦` StartedAt `2026-07-04T00:38:06Z` **unchanged** |
...
| Production | **Not modified** â€” FROZEN; prod API `65b4b9d4â€¦` StartedAt `2026-07-11T10:51:44Z` unchanged |
...
| AI Policy / provider routing | **Docs alignment COMPLETE (2026-07-12)** â€” [`AI_POLICY_PROVIDER_ROUTING.md`](./architecture/AI_POLICY_PROVIDER_ROUTING.md); OpenRouter = preferred text broker/adapter; direct OpenAI image adapter under same AI Policy; **not** universal routing implemented; BFL successful generation **not** claimed |
| VPS evidence | `/opt/dca/apps/dcaosv1/staging/backups/STAGING_DEPLOY_A8A74E6_RESULT_20260712-043322.txt` |
| Next sequence | WordPress one-draft COMPLETE â†’ MI (GA4/GSC **WITHDRAWN**) |
...
**Result:** KEEP â€” Orchestrator Lite **CONFIG SHAPE PROVEN** (not `STAGING LIVE PROVEN`; planâ†’execute not claimed).

| Item | State |
|------|--------|
| Staging artifact | `a8a74e6` |
| Marker | `DCA-WS7-AI-A-20260712-064227` |
| Entry point | Admin-only `GET /ai-orchestrator-lite/registry`, `POST â€¦/material-routing-preview`, `POST â€¦/workflow-dry-run` |
| Unauth denial | 401 on all three routes |
| Registry | Puriva cap `$100`; `orchestratorLiveSafe=true`; no live providers enabled |
| Routing matrix | research_pack, article_draftâ†’content_draft, report_narrativeâ†’workflow_summary, compliance allowLive=false, image_generation blocked, unknown blocked |
...
**Result:** KEEP â€” AI Delivery / OpenRouter path **STAGING LIVE PROVEN** (not production live; not Orchestrator planâ†’execute; not all AI workflows).
...
| Internal path | AI Delivery workflow â†’ `AI_GATEWAY_V1` â†’ `openrouter-text.service` |
...
**Result:** KEEP â€” R2 / private storage **STAGING LIVE PROVEN** for one isolated staging object (not production; not image storage; not client deliverable storage; not public delivery).
...
| Path | `uploadR2Object` â†’ `headR2Object` â†’ signed GET â†’ `deleteR2Object` â†’ `headR2Object` absence |
| Object key | `tenants/dca-r2-proof/years/2026/projects/dca-r2-20260712t081648z-cc7ee7/months/07/documents/â€¦pdf` |
...
| Next gate | Image provider / live generation proof (COMPLETE 2026-07-12 â€” OpenAI one-image) |

## OpenAI Images staging one-image live proof closeout (2026-07-12)

**Result:** KEEP â€” OpenAI Images staging one-image AI Policy path **STAGING LIVE PROVEN** (one bounded neutral image only; not three-image sets; not regeneration; not client/WordPress/publication; not production).
...
| Path | AI Policy â†’ `image_single` â†’ `ImageProviderAdapter` â†’ `OpenAIImageAdapter` â†’ OpenAI Images API |
...
| Output | `image/png`; 1213472 bytes; 1024Ã—1024; sha256 `5892cadcf699d1607e77ae72084780ca8d7f519968715ecdbe630e4c4c0da82f` |
...
**Result:** KEEP â€” WordPress dedicated staging one-draft path **STAGING LIVE PROVEN** (bounded one-draft create-and-trash only; not general publication; not image attach; not production).
...
| Path | staging proof harness â†’ Prisma attempt store â†’ `createWordPressDraft` â†’ one REST create â†’ exact-ID trash |
...
| Historical AMBIGUOUS | Three prior 401 attempts retained unchanged (markers `â€¦432278b0`, `â€¦ef73ba7e`, `â€¦02ab52be`) |
...
**Result:** KEEP â€” AI Delivery content-to-draft bounded workflow **LOCAL IMPLEMENTED / FAKE-PROVIDER PROVEN**.

| Item | State |
|------|-------|
| Path | approved content draft â†’ one fake OpenAI image â†’ one deterministic private fake storage object â†’ `AiDeliveryArticleImage.PREVIEW_READY` â†’ durable `WAITING_FOR_IMAGE_APPROVAL` â†’ existing image approval status â†’ one fake WordPress draft â†’ one fake owner/admin email â†’ `COMPLETED` |
...
**Result:** KEEP â€” AI Delivery content-to-draft execution bridge **LOCAL IMPLEMENTED / FAKE-PROVIDER AND REAL-PRISMA PROVEN**.
...
**Result:** KEEP â€” connected staging live proof **COMPLETE / PASS**; product API storageKey-only approve previously **NOT_PROVEN**, now **CLOSED locally** (staging re-proof after deploy).

| Item | State |
|------|--------|
| Bounded staging live proof | **COMPLETE / PASS** â€” prepare â†’ Stage A (1Ã— OpenAI `gpt-image-1` + 1Ã— private R2) â†’ owner approval of exact image ID â†’ Stage B (1Ã— WP draft + 1Ã— email) â†’ idempotency â†’ exact cleanup |
| Proof correlation | `c0cc2bd5-f6ff-4151-bccc-97b1b3222198` |
| Workflow run / article image | `7648ac50-â€¦` / `10422e0c-â€¦` |
| Proof-only recipient override | Commit `25f846e` â€” CLI/env override of Stage B `ownerRecipient.email` only; normal app email resolution unchanged |
| Owner approval of exact image ID | **PROVEN** (owner chat) |
| Product API `/approve` during that live proof | **NOT_PROVEN** â€” Stage A had `storageKey` only; URL-only preview gate blocked product approve; Stage B used a status-only DB update (not product-API proof) |
...
| Narrow staging product-approve proof | **PROVEN** on staging `57f1881` â€” correlation `approve-gap-a637cdad-â€¦`; product `POST â€¦/approve` â†’ `APPROVED`; URLs remained null; no direct DB status update; OpenAI/WP/email = 0; exact R2+DB cleanup residual 0 |
...
| Production | **Frozen** â€” unchanged |
| Deferred non-blocking | In-system notification E2E; production release â€” email remains priority notification path. **GA4/GSC: WITHDRAWN (not deferred).** |

## Staging email one-send proof closeout (2026-07-12)

**Result:** KEEP â€” **STAGING PROVIDER ACCEPTANCE PROVEN** (not inbox/webhook `STAGING LIVE PROVEN`).
...
**Result:** PASS â€” controlled staging deployment of commit `1b8d00d` completed; post-deploy smoke PASS; production untouched. **Superseded as current staging runtime by `a8a74e6` (2026-07-12); retained as rollback target.**
...
| Caddy | Not restarted or changed |
| Production | **Not modified** â€” production remains frozen |
| Production health during deploy | HTTP 200 |
| Post-deploy smoke | PASS |
| Source code / schema / migration change in repo | **None** â€” migrations were already in repo; only applied to staging DB |
...
## WORKSTREAM 1 â€” Points 1â€“4 closeout (2026-07-12)

**Result:** Workstream 1 Points 1â€“4 COMPLETE at `main` @ `250e958`. Staging artifact `1b8d00d` remains known-good PASS. Production remains FROZEN. No commit/push/deploy authorized by this status update alone.
...
| Point 1 â€” Vite | **COMPLETE** â€” Vite `6.4.3`; high finding closed; validate PASS; commit `95af080` |
| Point 2 â€” Canonical component system | **COMPLETE** â€” public `apps/web/src/components/ui`; private `apps/web/src/design-system` |
| Wave 0 â€” import guard | **COMPLETE** â€” commit `250e958`; historical freeze 108; **current baseline 0** after Modal Wave; new = 0 |
| Point 3 â€” Rollback plan | **COMPLETE** â€” verdict `ROLLBACK READY WITH CONDITIONS`; rehearsal **not executed** (separately gated); no SHA-tagged retained staging API image yet |
| Point 4 â€” Orchestrator proof decision | **COMPLETE** â€” `HYBRID â€” PREFLIGHT + EMBEDDED LIVE PROOF`; Orchestrator remains `LOCAL FOUNDATION`; not `STAGING LIVE PROVEN` until planâ†’execute is wired |
| Canonical component doc | [`CANONICAL_COMPONENT_SYSTEM.md`](./project-control/CANONICAL_COMPONENT_SYSTEM.md) |
| Wave 1 â€” first consolidation slice | **COMPLETE** â€” ui barrel exports Tabs/compound Table/ActivityItem; unused root state shims deleted |
| Modal Wave | **COMPLETE** â€” canonical `components/ui/Modal`; 23/23 page consumers migrated; legacy root Modal deleted; import-guard baseline 0; impl `a447b9e`; docs-closeout `67d9aa4`; staging web-only artifact `a447b9e` + Modal browser proof PASS |
| Modal staging Caddy | **SHARED_PROXY_RESTART (historical)** â€” Modal web-only deploy remounted staging static bind; production app/API/DB unchanged. Prefer future **in-place sync** into `/opt/dca/apps/dcaosv1/staging/web/dist` (preserve mount inode; no Caddy recreate). |
| Remaining UI migration | **PARTIAL** â€” Card adapter DEFERRED; DS Modal `aria-describedby` DEFERRED (requires DS edit); WP-over-Deliverables Escape stack mitigated by suspending Deliverables `isOpen` while confirm open; state primitives + typography floors advanced in RC convergence |
| Next local Workstreams | **2â€“5** before staging Workstream 6 |
| Production | FROZEN |
...
**Result:** PASS â€” explicit `Client.operatingPackKey` binding + fail-closed resolver. Pack content remains code-defined in `@dca-os-v1/shared`. No silent Puriva default for unbound clients. Production migration/deploy remain owner-gated.
...
| Canonical binding key | `PURIVA_OPERATING_PACK_V1` â†’ registry packKey `puriva` |
| Resolver | `apps/api/src/core/client-operating-pack.resolver.ts` â€” `resolverSource=database_binding` |
...
| Staging | Controlled migrate + API/web deploy **COMPLETE** on `57f9c52`; artifact `/opt/dca/staging-artifacts/57f9c52`; migration applied; binding smoke 11/11; durable Puriva client `3afb9e45-â€¦` bound to `PURIVA_OPERATING_PACK_V1` |
| Production | **FROZEN** â€” separate migrate/deploy approval |

## Puriva staging redeploy + composition closeout (2026-07-13)

**Result:** PASS â€” controlled staging redeploy of `632d9a9` (Puriva Operating Pack runtime) + staging composition proven. Migration SKIP. Shared Caddy **not** restarted. Production app/API/DB/web unchanged. Live provider flags remain false. Full Puriva Launch READY is **not** claimed. **Superseded for client DB binding by the closeout above.**
...
| Staging API | Recreated â€” id `ca684230â€¦` StartedAt `2026-07-13T04:08:51Z` |
| Staging DB | Unchanged â€” id `c70f523dâ€¦` StartedAt `2026-07-04T00:38:06Z` |
| Web deploy | In-place sync into `/opt/dca/apps/dcaosv1/staging/web/dist`; assets `index-DY5z8SCD.js` / `index-i_JTDDN4.css` |
| Shared proxy | `SHARED_PROXY_ACTION=none` â€” Caddy id/StartedAt unchanged (`fe598a24â€¦` / `2026-07-13T02:26:36Z`) |
| Production API/DB | Unchanged (`65b4b9d4â€¦` / `db95badaâ€¦`) |
| Live flags | `AI_TEXT_GATEWAY` present (masked); image/WP draft/email live / publish flags absent or false |
| Pack runtime | **PASS** â€” `getClientOperatingPackConfig("puriva")` in staging API image; `$100` cap; EN primary; staging WP host; review/fallback profiles |
| Client DB binding | **NOT_PROVEN** â€” convention/shared constants (no product DB pack-binding claim) |
| Compliance staging | **PASS** â€” runtime assessor blocks/flags guarantee/cure/fake doctor/patient/before-after/BPOM/superiority/authority; allows cautious educational |
| AI budget staging | **PASS** â€” `PURIVA_AI_MONTHLY_CAP_USD=100` in staging runtime |
| Admin / client roles | **PASS** â€” composition smoke product APIs |
| Cross-tenant staging | **PASS** â€” real foreign client/project/report/deliverable; Purivaâ†”foreign 403/404 fail-closed |
| Monthly report staging | **PASS** â€” fixture/non-live metrics only; `MONTHLY_REPORT_LIVE_GA_GSC=DEFERRED` |
| Client portal browser | **PASS** â€” `smoke-puriva-staging-browser.mjs` 29/29; desktop+mobile; expected client 403s on admin routes |
| Composition smoke | `smoke-puriva-staging-composition.mjs` **49/49**; correlation `puriva-staging-composition-5cc345e5-â€¦` |
| Browser correlation | `puriva-staging-browser-3b2a26e5-â€¦` |
...
**Result:** PASS â€” `DOCKER NON-ROOT STAGING PASS`. The application image now uses the existing `node` user, local image/runtime proof passed, controlled staging deployment of the exact authorized commit passed, and no permission or restart regression was observed. Production was not redeployed.
...
**Result:** PASS â€” controlled staging deploy of `9921bb3` (storageKey preserve + UI state/readability convergence + docs truth). Migration SKIP. Shared Caddy **not** restarted. Production app/API/DB unchanged. **Superseded as current staging runtime by `632d9a9` Puriva redeploy above.**
...
| Staging API | Recreated â€” id `c1a9fc1eâ€¦` StartedAt `2026-07-13T03:07:22Z` |
| Staging DB | Unchanged â€” id `c70f523dâ€¦` StartedAt `2026-07-04T00:38:06Z` |
| Web deploy | In-place sync into `/opt/dca/apps/dcaosv1/staging/web/dist` (inode preserved); index sha256 `fb3e4bddâ€¦` |
| Live assets | `index-DY5z8SCD.js` / `index-i_JTDDN4.css` |
| Shared proxy | `SHARED_PROXY_ACTION=none` â€” Caddy id/StartedAt unchanged |
| Production API/DB | Unchanged (`65b4b9d4â€¦` / `db95badaâ€¦`) |
...
| Puriva Operating Pack | Assembled in `@dca-os-v1/shared` â€” identity, tone, languages, image dimensions, WP targets (incl. staging host), review rules, fallbacks, `$100` cap |
...
| Local monthly rehearsal | `node scripts/smoke-puriva-monthly-rehearsal-local.mjs` â€” **29/29 PASS**; correlation `puriva-rehearsal-d7806bec-â€¦` |
...
| GA4/GSC live | **WITHDRAWN** â€” live GA4/GSC not in scope; MANUAL/snapshot monthly reports remain |
| In-system notification E2E | **DEFERRED** |
| Staging redeploy | **COMPLETE** on `632d9a9` â€” see Puriva staging redeploy + composition closeout above |
| Production | **FROZEN** â€” `BLOCKED_OWNER_ACTION=APPROVE_PRODUCTION_DEPLOY` |
...
## WORKSTREAM 1A â€” project control reconciliation (2026-07-12) â€” historical

**Result:** Docs-only. Authoritative project-control matrix recorded. Superseded for Points 1â€“4 by WORKSTREAM 1 closeout above. No app/schema/staging/production mutation in 1A itself.

| Item | State |
|------|--------|
| Repo baseline (at 1A) | `main` @ `fac108be16e779fdbff0a2867b302679c8c4da6f` |
| Staging baseline | `1b8d00d` PASS (unchanged) |
| Relation (at 1A) | `1b8d00d â†’ fac108b` (docs-only descendant) |
| Numbering systems | BLOKI 1â€“13 Â· Fazy UI 1â€“13 Â· Workstreamy 1â€“9 Â· G-gates (separate; do not mix) |
...
| Dependency finding (at 1A) | Vite high was OPEN â€” later closed by Point 1 / `95af080` |
| Open decisions (at 1A) | Later closed for canonical / Orchestrator proof / rollback **plan**; rehearsal and Waves 1â€“5 remain open |
| Production | FROZEN |
| Canonical doc | [`AUTHORITATIVE_PROJECT_CONTROL_MATRIX.md`](./project-control/AUTHORITATIVE_PROJECT_CONTROL_MATRIX.md) |

**Historical note:** Older G-gate rows below that still say â€œno notification migration/inboxâ€ are **historical context** from the date of that gate. Current ground truth is the WORKSTREAM 1 closeout section and the authoritative matrix.

## G77b persistent COMPLETED ledger live proof closeout (2026-07-10)

| Item | State |
|------|--------|
| Gate | G77b â€” controlled local live proof that OpenRouter AI Delivery execute persists COMPLETED `AiBudgetLedgerEntry` |
| Preflight commit | `9ba707f` â€” `test: add openrouter api env preflight` |
| Operator | Piotr Pakula |
| Target | Local only â€” `127.0.0.1:4000` |
| No-live API env preflight | **PASS** â€” `smoke:openrouter-api-env-preflight:local` 18/18 |
| Phase 1 â€” baseline local guarded smoke | **PASS** â€” 12/12; `liveProviderCalled=false` |
| Phase 2 â€” one live OpenRouter guarded smoke | **PASS** â€” 12/12; exactly one live call |
| Formal live `workflowRunId` | `2244413e-d87b-45a1-8a26-6634ec8972d5` |
| Provider / model | OpenRouter â€” `anthropic/claude-haiku-4.5` |
| Phase 3 â€” restore local guarded smoke | **PASS** â€” 12/12 |
| Ledger verifier | **PASS** â€” persistent COMPLETED row found |
...
| `actualCostUsd` | `null` â€” **expected current limitation**; does not prove provider invoice cost |
| Forbidden integrations | **None triggered** |
| Secrets | **Not exposed** |
| Staging / VPS / production / deploy | **Not touched** |
| Production readiness | **NO** â€” production remains frozen; staging/production live proof **not** claimed |
...
Detail: [`AI_MODEL_ROUTING_POLICY.md`](./runbooks/AI_MODEL_ROUTING_POLICY.md) Â· [`AI_PROVIDER_LIVE_PROOF.md`](./runbooks/AI_PROVIDER_LIVE_PROOF.md) Â§9.18 Â· [`deferred-scope-register.md`](./operator/deferred-scope-register.md).

## G78 multi-scope docs alignment (2026-07-10)

| Item | State |
|------|--------|
| Gate | G78 â€” post-G77b clarity, readiness labels, and future-scope consolidation (docs only) |
...
| Live AI / OpenRouter | **Not executed in G78** â€” G77b evidence preserved |
| Production / staging / deploy | **Not touched** â€” production remains frozen |
...
Detail: [`deferred-scope-register.md`](./operator/deferred-scope-register.md) (future scope buckets) Â· [`notifications-blocker-plan.md`](./operator/notifications-blocker-plan.md) Â· [`CLIENT_OPERATING_PACKS.md`](./architecture/CLIENT_OPERATING_PACKS.md) Â§14 Â· [`INTEGRATIONS_TRUTH_MATRIX.md`](./runbooks/INTEGRATIONS_TRUTH_MATRIX.md).

## G79-G88 multitask consolidation (2026-07-10)

> **Historical context (superseded for notifications):** G82 row below recorded the 2026-07-10 planning state (â€œno in-system notification modelâ€). Current ground truth (2026-07-12): in-app notification persistence/UI = LOCAL FOUNDATION; staging migration APPLIED on `1b8d00d`; live email still not STAGING LIVE PROVEN â€” see WORKSTREAM 1A and [`AUTHORITATIVE_PROJECT_CONTROL_MATRIX.md`](./project-control/AUTHORITATIVE_PROJECT_CONTROL_MATRIX.md).

| Gate | Result | Evidence / limitation |
|------|--------|-----------------------|
| **G79 â€” Monthly cap aggregation for live COMPLETED rows** | **READY (local implemented)** | `sumSpentUsdForPeriod()` now includes live `COMPLETED` rows and sums `actualCostUsd ?? estimatedCostUsd`; focused unit tests 18/18 PASS on `ai-budget-ledger.service.test.ts` |
| **G80 â€” `actualCostUsd` policy** | **PARTIAL (policy only)** | Policy documented: `actualCostUsd` stays `null` unless trusted provider cost is exposed; trusted provider-cost ingestion remains deferred |
| **G81 â€” Staging live proof planning** | **PARTIAL (planning only)** | `AI_PROVIDER_LIVE_PROOF.md` has a staging plan; no staging execution or live call occurred |
| **G82 â€” Notifications foundation planning** | **BLOCKED (not started in-system)** *(historical as of 2026-07-10)* | No in-system notification model exists *at G82 date*; staged plan updated in `notifications-blocker-plan.md` |
| **G83 â€” Email notification no-send proof** | **PARTIAL (no-send planning)** | `EMAIL_NOTIFICATIONS_PROOF.md` no-send checklist updated; no live email sent |
| **G84 â€” Client approval notification loop event map** | **PARTIAL (event map)** | Approval event map documented; image-level approve/reject has no notification intent; monthly report `FINAL` is admin-only and not client delivery |
| **G85 â€” GA/GSC live metrics planning** | **BLOCKED (live data)** | `MONTHLY_REPORT_LIVE_DATA_PROOF.md` and Puriva monthly gates separate MANUAL snapshot placeholders from live GA/GSC; no OAuth/live sync exists |
| **G86 â€” WordPress live draft proof planning** | **PARTIAL (planning only)** | `WORDPRESS_DRAFT_PROOF.md` three-tier plan documented; live draft proof not executed; publish remains frozen |
| **G87 â€” Image generation provider proof planning** | **BLOCKED (provider/live proof)** | `IMAGE_GENERATION_PROOF.md` medical-aesthetic and future proof checklist updated; no provider selected or called |
| **G88 â€” Puriva launch readiness consolidation** | **BLOCKED (launch unchanged)** | Shared docs consolidated with the correct gate map; Puriva Launch remains blocked and production remains frozen |
...
**Recommended next gate:** **G89** â€” owner-selected launch-blocker execution gate, preferably the lowest-blast-radius proof that unlocks downstream work (R2 real-bucket proof is the current recommended first candidate). Any live proof, staging/prod action, commit, push, or deploy still requires explicit owner approval.
...
| **G100** | **BLOCKED** *(historical as of G89â€“G148 / 2026-07-10)* | Notification persistence/in-system inbox remained deferred **at that gate**; **superseded 2026-07-12** â€” persistence/UI LOCAL FOUNDATION; staging migration APPLIED; see WORKSTREAM 1A |
| **G101** | **BLOCKED** | Live email/Resend proof remains deferred (**still current** â€” not STAGING LIVE PROVEN) |
...
**Recommended next gate:** **G149** â€” owner-selected launch-blocker execution gate, with R2 target-environment real-bucket proof still the recommended first low-blast-radius candidate. See [`docs/operator/G147_NEXT_20_GATES.md`](./operator/G147_NEXT_20_GATES.md).
...
|------------|--------|--------------------------------|
| **G149â€“G158** Storage/R2 | **READY (local)** | Proof-stage hardening, redacted config summary, private-storage proof intent, client-safe URL policy, serializer storageKey boundary tests, cleanup proof-plan constants; **no real bucket IO** |
| **G159â€“G170** Notifications | **READY (local) / BLOCKED (inbox+live)** *(historical persistence claim)* | Expanded taxonomy V2, recipient/channel/severity/redaction helpers, typed templates, no-send edge tests; persistence/inbox **design only** â€” no migration **at G149â€“G228 date**; **superseded 2026-07-12** (persistence implemented; live email still deferred) |
| **G171â€“G180** GA/GSC/monthly | **READY (local) / BLOCKED (live)** | Credential presence, period policy, source-truth serializer, metric validation, recommendation policy, client/admin output guards, CSV import proof plan; **no live Google** |
| **G181â€“G188** WordPress | **READY (local) / BLOCKED (live)** | Draft payload hardening, slug policy, status freeze, credential redaction, proof-plan constants, image-inclusion contract; **no live HTTP** |
| **G189â€“G198** Image | **READY (local) / BLOCKED (live)** | Compliance V2, prompt/alt/reject/approval-loop helpers, WP inclusion readiness, provider proof plan; G228 added four image-loop events to shared taxonomy; **no live provider** |
| **G199â€“G208** Client Portal | **READY (local) / BLOCKED (staging)** | Serializer/archive/monthly guards, approval serializers, pure approval policy, error-safety helpers, route inventory docs; one-revision-round needs durable schema field |
| **G209â€“G216** Puriva pack | **READY (local)** | Entitlement matrix, compliance validator, workflow catalog, client-visibility helpers, saas-later truth docs |
| **G217â€“G222** Future modules | **READY (contracts)** | MI/Revenue Hub/POD contracts hardened; no live scrape/CRM/marketplace |
| **G223â€“G227** Security/roadmap | **READY (docs)** | Security checklist G223, smoke inventory, validation guards, deferred cleanup proposal, [`G227_NEXT_30_GATES.md`](./operator/G227_NEXT_30_GATES.md) |
| **G228** | **KEEP pending validation** | Main-agent integration, cross-lane image notification taxonomy join, validation, final report |

**Recommended next gate:** **G229** â€” owner-selected launch-blocker execution gate; R2 target-environment real-bucket proof remains the recommended first low-blast-radius candidate. See [`docs/operator/G227_NEXT_30_GATES.md`](./operator/G227_NEXT_30_GATES.md).
...
| Gate range | Status | Integrated result / limitation |
|------------|--------|--------------------------------|
| **G229â€“G248** Storage/R2 | **READY (local)** | Proof-stage edges, partial-config diagnostics, no-IO label invariant, cleanup plan, proof-intent, client-safe URL policy, storageKey boundary, admin-vs-client field policy, error redaction; **no real bucket IO** |
| **G249â€“G268** Notifications | **READY (local) / BLOCKED (inbox+live)** *(historical persistence claim)* | Taxonomy aliases, payload snapshots, correlation/idempotency design, no-send/template/config tests; persistence/inbox **design only** â€” no migration **at G229â€“G468 date**; **superseded 2026-07-12** (persistence implemented; live email still deferred) |
| **G269â€“G288** GA/GSC/monthly | **READY (local) / BLOCKED (live)** | Config/redaction/period/source-truth, unavailable-state + export-truth helpers, output guards; **no live Google** |
| **G289â€“G308** WordPress | **READY (local) / BLOCKED (live)** | Slug/draft/freeze/redaction, sanitization, taxonomy placeholders, author-tenant mapping design; **no live HTTP**; publish frozen |
| **G309â€“G328** Image | **READY (local) / BLOCKED (live)** | Compliance V3, prompt/alt/approval-loop, no-live provider invariant; **no live provider** |
| **G329â€“G348** Client Portal | **READY (local) / BLOCKED (staging)** | Serializer/error/approval no-leak tests, route inventory, admin-vs-client matrix; revision-round persistence still needs schema |
| **G349â€“G368** Puriva pack | **READY (local)** | Entitlement/visibility/catalog helpers, workflow templates (`executionEnabled: false`), saas-later truth labels |
| **G369â€“G388** Future modules | **READY (contracts)** | MI/RH/POD contract hardening; no scrape/CRM/marketplace/image |
| **G389â€“G408** AI budget/routing | **READY (local)** | Reporting/reconciliation, trusted-actual design, notification mapping to existing `BUDGET_*`, routing truth labels; `actualCostUsd` ingestion deferred |
| **G409â€“G428** Security/operator | **READY (docs)** | Checklist G409, redaction inventories, freeze/staging sweeps, no-live catalogue, owner/live-proof checklists |
| **G429â€“G448** UI/testability | **READY (docs + tiny copy)** | Surface inventories, hotspot split plan, proof-state vocabulary/labels; no large UI rewrite |
| **G449â€“G468** Deferred/roadmap | **KEEP** | Deferred register cleanup, truth matrix/Puriva/STATUS refresh, [`G468_NEXT_50_GATES.md`](./operator/G468_NEXT_50_GATES.md) |

**Recommended next gate:** **G469** â€” owner-selected launch-blocker execution gate; R2 target-environment real-bucket proof remains the recommended first low-blast-radius candidate. See [`docs/operator/G468_NEXT_50_GATES.md`](./operator/G468_NEXT_50_GATES.md).

## G134-G137 AI budget reporting/reconciliation contract (2026-07-10)

| Gate | Result | Evidence / limitation |
|------|--------|-----------------------|
| **G134 â€” AI budget vs Finance Lite separation** | **READY (contract/docs/tests)** | `ai-budget-reporting.contract.ts` defines a separate AI budget reporting boundary; Finance Lite remains admin finance records/invoices/bills only |
| **G135 â€” Reconciliation design** | **READY (design only)** | Estimate, trusted actual, invoice, and variance slots are modeled; invoice fields remain `null` / `not_integrated`; no real invoice source is wired |
| **G136 â€” Reporting contract** | **READY (local unit proof)** | Contract reports monthly cap, countable/live rows, estimated-vs-actual spend basis, provider/model breakdown, and G79-compatible spend math without changing `sumSpentUsdForPeriod()` |
| **G137 â€” Docs closeout** | **READY (docs)** | Detail recorded in [`AI_BUDGET_REPORTING_RECONCILIATION_CONTRACT.md`](./runbooks/AI_BUDGET_REPORTING_RECONCILIATION_CONTRACT.md) and [`AI_MODEL_ROUTING_POLICY.md`](./runbooks/AI_MODEL_ROUTING_POLICY.md) |
...
| Item | State |
|------|--------|
| Gate | G76b + G76c + G76d â€” persistent COMPLETED `AiBudgetLedgerEntry` wiring (mocked/no-live proof) |
...
| Persistence path | Successful OpenRouter AI Delivery workflow execute â†’ `recordCompletedAiLedgerEntry()` via existing G74 helpers |
...
| Schema | **No change** â€” existing `AiBudgetLedgerEntry` sufficient |
| Validation | `test:unit` 252/252 PASS; `validate` PASS; `git diff --check` PASS |
| Live proof | **DONE in G77b (local only)** â€” superseded the G76 â€œNOT DONEâ€ deferral; see G77b closeout above |
...
Detail: [`AI_MODEL_ROUTING_POLICY.md`](./runbooks/AI_MODEL_ROUTING_POLICY.md) Â· [`AI_PROVIDER_LIVE_PROOF.md`](./runbooks/AI_PROVIDER_LIVE_PROOF.md) Â§9.17 Â· [`deferred-scope-register.md`](./operator/deferred-scope-register.md).

## G75 live spend attribution proof closeout (2026-07-10)

| Item | State |
|------|--------|
| Gate | G75 + G75c + G75d â€” controlled live spend attribution proof (local only) |
| `main` commit (implementation base) | `e5269fc` â€” `feat: add AI completed ledger attribution` |
| Operator | Piotr Pakula |
| Phase 1 â€” local deterministic baseline | **PASS** â€” `smoke:openrouter-guarded:local` 12/12; `liveProviderCalled=false` |
| Phase 2 (G75 v2) â€” one live OpenRouter proof | **PASS** â€” strict live smoke 12/12; exactly **one** live OpenRouter call |
...
| Phase 3 â€” restore local deterministic gateway | **PASS** â€” guarded smoke 12/12; `liveProviderCalled=false` after restore |
| G75c completed attribution verifier | **PASS** â€” `node --import tsx $env:TEMP\verify-g75-completed-attribution-v2.mjs`; completed attribution metadata generated via G74 `finalizeOrchestratorLiteLedgerAttribution` against live observability |
| Persistent COMPLETED ledger row | **NOT FOUND** â€” smoke path does not auto-persist completed attribution; metadata generated-only at verify time; **deferred to G76** |
| G75c root cause | Verifier/tooling only â€” plain `node` could not resolve shared barrel `.ts` re-exports; original verifier selected post-restore local client instead of live OpenRouter evidence |
| Forbidden integrations | **None triggered** |
| Secrets | **Not exposed** |
| Production readiness | **NO** â€” production remains frozen; staging/production live proof not claimed |
| Next gate | **G75e** commit/push docs closeout; then **G76** persistent live completed ledger row wiring |

Detail: [`AI_MODEL_ROUTING_POLICY.md`](./runbooks/AI_MODEL_ROUTING_POLICY.md) Â· [`AI_PROVIDER_LIVE_PROOF.md`](./runbooks/AI_PROVIDER_LIVE_PROOF.md) Â§9.16 Â· [`deferred-scope-register.md`](./operator/deferred-scope-register.md).
...
| Latest implementation commit | `64bfd06` â€” `prelive: complete post-G56 orchestration readiness` (G57â€“G68 merged via G69 fast-forward) |
| G69 merge | **DONE** â€” branch `cursor/g57-g68-prelive-completion` fast-forward to `main`; validation before merge PASS (198 unit tests, orchestrator + provider smokes, validate) |
| Working tree | Clean except untracked `.cursor/settings.json` (must not be committed) |
| Latest docs/local closeout commit | `42f969f` â€” `docs: mark G54 HSTS blocker closed in G53 plan`; G35 Phase B local smoke proof remains `217c11c`; current staging artifact is `1b8d00d` after 2026-07-11 controlled deploy |
| Pre-staging local closeout (G35 Phase B) | **PASS** â€” full local pre-staging gate passed on `217c11c`; see Â§2.7 |
| Latest local pre-staging re-check (G43) | **PASS** â€” validate plus four focused local smokes passed on current `main`; no repo edits, commit/push/deploy, staging/VPS/prod; see Â§2.9 |
| Controlled refresh (G35 Phase C) | **PASS** â€” staging artifact refreshed from `5ee8389` to `5e1ea5a`; local validate PASS before artifact creation; staging API recreated; DB healthy; MVP smoke PASS; production untouched; see Â§2.8 |
| Production deploy | **Frozen/deferred; production deploy ready: NO** â€” `system.digitalcubeagency.net` unchanged; G48/G53 planning PASS do not authorize deploy. G49 dry-run and G50 deploy **not executed**. G54 HSTS/proxy: PASS. Next production path remains G49 dry-run before G50. |
...
| Default AI execution | Local deterministic (restored after G71e-retry); live OpenRouter opt-in only â€” **local formal proof complete** |
...
## 2. Latest closed blocks (Blocks 1â€“4)

| Block | Commit | Scope | CI |
|-------|--------|-------|-----|
| **1** â€” External integrations readiness | `136e93a` | Config-only readiness layer: `GET /api/v1/integrations/readiness`, `check:external-integrations-readiness`, `smoke:external-integrations-readiness:local`. No live calls, publish, sync, or bucket IO. | Green |
| **2** â€” Admin operations | `5308f19` | Read-only `GET /api/v1/admin/operations/summary`, dashboard Operational readiness panel, recovery runbook, `smoke:admin-operations:local`. | Green |
| **3** â€” UI density | `cc40160` | Dark-theme UI density consolidation for admin and client surfaces (CSS-only). No backend/schema/API/auth changes. | Green |
| **4** â€” Docs + operator runbook | `c7af674` | Consolidate STATUS, architecture map, smoke matrix, operator runbooks, env inventory, staging gate, deferred roadmap. Docs-only. | Green |
| **5Aâ€“5D-A** â€” Claude audit remediation | `2437c84` â€¦ `e54445f` | Admin tenant-read RBAC, fail-closed client boundary smokes, CI unit-test proof, cross-platform test globs, remote staging opt-in, bootstrap guard hardening. | Green |
...
### 2.1 Block 5D-B â€” pre-staging local closeout (2026-07-05)
...
| 2 Audit / Block 1â€“2 smokes | PASS | External integrations readiness; admin operations 16/16; client-role API boundary 48/48 |
| 3 Block A core smokes | PASS (manual fallback) | Puriva client portal boundary 153/153; remaining core smokes run individually â€” all PASS |
...
**Audit remediation commits (5Aâ€“5D-A, on `main`):**
...
**Known issue:** `npm run smoke:staging-readiness:local` orchestrator can hang after Puriva smoke completes (local PowerShell/log/process handling). Manual fallback for remaining Block A core smokes succeeded. Before G4, fix orchestrator or explicitly accept the manual workaround â€” see [`STAGING_READINESS.md`](./runbooks/STAGING_READINESS.md).
...
### 2.2 G35 Phase C â€” controlled staging refresh proof (2026-07-07)

**Controlled refresh on commit `5e1ea5a`:** staging artifact refreshed from `5ee8389` to `5e1ea5a` (`docs: record staging discovery facts`). Local validation PASS before artifact creation. Staging API recreated; DB healthy (not recreated). Staging MVP smoke PASS. Production containers untouched. Full evidence recorded in Â§2.8.
...
| MVP smoke result | PASS â€” login/auth/modules/logout/token boundary |
...
**Result:** PASS â€” client portal approval UX closeout on `ec4c41c` (`polish(client): clarify Puriva approval UX`).
...
**Result:** PASS â€” G6 Wave 1 complete on `4eeac1e` (`feat(ops): add G6 Puriva launch cockpit and operating pack`).
...
**Result:** PASS â€” G6 Wave 2 complete on `95a92c9` (`polish(ops): compact delivery handoff lanes`).
...
| Post-push verification | PASS â€” `main` synced with `origin/main`; remote `origin/main` points to `95a92c9`; working tree clean |
| Production / staging / VPS | Untouched |
...
**Result:** PASS â€” Puriva intake now connects to AI Knowledge, WorkflowBriefs, SEO planning, and AI Delivery handoff on `8cb41e2` (`polish(ops): connect Puriva intake to delivery path`).
...
| CI | Green |
| Push | PASS â€” `main` synced with `origin/main` after commit/push |
| Boundaries preserved | Admin-only / client-safe / draft-only boundaries preserved; no backend/schema/auth changes |
| Production / staging / VPS | Untouched |
| Next block | Owner decision |

### 2.7 G35 Phase B â€” browser smoke stabilization closeout (2026-07-07)

**Result:** PASS â€” local pre-staging gate closed on `217c11c` (`test: stabilize G35 Phase B browser smokes`).
...
| CI | Green on `217c11c` |
| Local gate | `npm.cmd run smoke:pre-staging:local` PASS â€” local repo gate complete |
...
### 2.8 G35 Phase C â€” controlled staging refresh closeout (2026-07-07)

**Result:** PASS â€” controlled refresh of staging on `5e1ea5a` (`docs: record staging discovery facts`). **Base:** `217c11c` Phase B proof (`test: stabilize G35 Phase B browser smokes`). **Scope:** docs-only discovery, local validation, artifact creation/upload, controlled VPS artifact refresh, staging API recreation, admin bootstrap verification, and MVP smoke pass.

| Item | Evidence |
|------|----------|
| Artifact source commit | `5e1ea5a` (`docs: record staging discovery facts`) |
| Local pre-artifact validation | PASS â€” `npm.cmd run validate` clean before artifact creation |
| Artifact creation + upload | Local tar: `dcaosv1-5e1ea5a.tar` â†’ remote temp `/tmp/dcaosv1-5e1ea5a.tar` â†’ extracted to `/opt/dca/staging-artifacts/5e1ea5a` |
| Staging compose context update | `5ee8389` â†’ `5e1ea5a` (verified `docker-compose.yml` points to new artifact) |
...
| Prisma migrate deploy | Not performed â€” no pending migrations detected for staging DB |
...
| Staging MVP smoke | Login 200, auth/me 200, modules/current 200, logout 200, reused token 401 â€” PASS |
...
| Deploy state | Controlled VPS artifact refresh only â€” no code push, no staging CI/migration deploy, no production touch |
| Boundaries preserved | Docs-only update + discovery facts recorded in STATUS.md Â§2.2; no runtime code changes, no schema changes, no secret exposure |

### 2.9 G38â€“G43 local/docs alignment closeout (2026-07-07)
...
**Result:** PASS â€” controlled staging deploy/proof completed against staging only. Production deploy attempted: **NO**. Production app/API/DB mutation: **NO**.
...
| Correct staging API compose service | `dcaosv1-staging-api` â€” not `api` |
...
**Result:** PASS â€” staging smoke/proof completed after G46d. Docs-only G47d records proof facts. No repo/source edits occurred during smoke gates. No deploy was attempted. VPS/staging/production were not mutated. No commit or push occurred during smoke gates.

| Item | Evidence |
|------|----------|
| Baseline commit before G47 docs | `f25158d` (`docs: record G46d staging deploy proof`) |
| G47 minimal staging proof | PASS â€” `staging-root-http=200`; `staging-health-http=200`; `prod-health-only-http=200`; staging/prod separation confirmed |
...
| G47c warning | HSTS missing â€” known proxy hardening item; warning only; production health probe inside smoke skipped unless explicitly approved |
...
**Historical G47c note:** G47c reported HSTS missing as warning only at that time. **G54 HSTS/proxy: PASS** â€” HSTS is now present on staging and production.

### 2.12 G48 production readiness planning closeout (2026-07-09)

**Result:** PASS â€” production readiness planning and sealed checklist alignment completed as a docs-only planning gate. **Production deploy ready: NO. Production deploy attempted: NO. Production remains frozen/deferred until separate explicit owner approval.**
...
**Result:** PASS â€” G53 production safety plan approved as docs-only planning gate. **Production deploy ready: NO. Production deploy attempted: NO. G53 does not authorize implementation, G54 fix, G49, G50, or production mutation.**

| Item | Evidence |
|------|----------|
| G53 approved | **YES** â€” planning only |
| Production readiness | **NO** |
| Production v1 principle | Controlled agency ops â€” admin-controlled, approval-gated; not full automation |
| Staging proven | G46d/G47 PASS from prior gates |
| G49 dry-run | **Not executed** |
| G50 production deploy | **Not executed** |
| Next gate | **G49** â€” production dry-run/read-only proof (not executed; owner approval required) |
| Puriva Launch | **Blocked** â€” live proof gates required before launch |
| RBAC stance | Not blocker for limited Production v1 if boundaries safe; blocker before scaling/SaaS |

**G53 blockers recorded:** HSTS/proxy security warning; rollback/restore evidence; env/secrets separation; credential storage; tenant/client boundary re-verification on target; integration truth matrix ([`INTEGRATIONS_TRUTH_MATRIX.md`](./runbooks/INTEGRATIONS_TRUTH_MATRIX.md) â€” audit baseline 2026-07-09); controlled dry-run (G49); G49 before G50 sequence.

**Next gates (ordered reference):** G49 production dry-run/read-only proof before any production deploy path. Puriva Launch proof gates remain separate: R2 proof â†’ AI Model Research â†’ AI Model Policy â†’ live AI proof ([`AI_PROVIDER_LIVE_PROOF.md`](./runbooks/AI_PROVIDER_LIVE_PROOF.md)) â†’ image generation proof â†’ transactional notifications proof. **Live GA4/GSC: WITHDRAWN â€” not a launch gate.** WordPress draft handoff: [`WORDPRESS_DRAFT_PROOF.md`](./runbooks/WORDPRESS_DRAFT_PROOF.md) (auto-publish remains deferred).
...
| **AI Delivery** | **100% local/operator-ready** | Full local operator sequence: project/month â†’ brief/context handoff â†’ workflow run visibility â†’ content plan â†’ content drafts â†’ reviews â†’ package â†’ deliverables â†’ WordPress draft-prep handoff â†’ monthly report/client-safe archive handoff | Live AI provider, live WP publish, live GA/GSC, live R2 IO, Google Docs live export, staging/environment proof, production readiness |
| **Workflow Briefs / context composition** | **100% local/operator-ready** | Intake â†’ submit â†’ approved KB/context â†’ MI/SEO runs â†’ production plan â†’ drafts â†’ AI Delivery handoff; AI SEO lives inside Workflow Briefs | Knowledge picker (6C-v2) and per-brief audit (6D) remain deferred; live provider, staging proof, and production readiness stay deferred |
...
| **AI SEO planning + content drafts** | **100% local/operator-ready** | WorkflowBriefs MI/SEO outputs â†’ production/content plan â†’ content objectives â†’ draft generation/review â†’ PDF/export handoff status â†’ AI Delivery handoff are locally smoke-proven for admin/operator use | Live crawling, GSC/GA sync, live provider execution, Google Docs live export, live R2 IO, live WordPress, staging/environment proof, production readiness |
...
| **Puriva Operating Pack v1** | **100% local/admin-operational pack** â€” **Puriva Client-Service Launch: BLOCKED** | Local/admin-operational closeout complete (workflow/docs maturity only, not launch readiness); first Client Operating Pack instance per [`CLIENT_OPERATING_PACKS.md`](./architecture/CLIENT_OPERATING_PACKS.md) Â§14; G79 local ledger aggregation implemented; G80 policy documented | Puriva Launch blocked until staging/production live proof gates and product workflow gates are clean; local live AI text + COMPLETED ledger row proven (G77b); staging/production re-proof still required; GA/GSC, R2, image gen, in-system + email notifications, production deploy remain deferred |
...
- `npm.cmd run validate` â€” prisma generate + check + build all workspaces
- `npm.cmd run smoke:external-integrations-readiness:local` â€” config shape only
- `npm.cmd run smoke:admin-operations:local` â€” admin summary + client boundary
- `node scripts/smoke-client-approval-happy-path-local.mjs` â€” when `AUTH_SEED_TEST_PASSWORD` set
- `npm.cmd run smoke:staging-readiness:local` â€” Block A focused subset
- `npm.cmd run smoke:production-readiness:local` â€” broad closeout (long; restarts API)
...
- AI SEO planning + content draft flow inside WorkflowBriefs and AI Delivery: MI/SEO report â†’ production plan â†’ content objectives â†’ drafts â†’ review/polish â†’ package/export handoff
...
## 7. Staging / production â€” G53 production safety plan approved; production deploy frozen
...
| Runtime separation | â€” | Shared Caddy `dca-caddy`; staging API `dcaosv1-staging-api` on `127.0.0.1:4011->4000`; staging DB `dcaosv1-staging-postgres` on `127.0.0.1:5435->5432`, healthy; production API `dcaosv1-api` on `127.0.0.1:4010->4000`; production DB `dcaosv1-postgres` on `127.0.0.1:5434->5432`, healthy. |
| Deploy proof | â€” | **G48 + G53 planning PASS only** â€” production deploy attempted NO; G49/G50 not executed; VPS/staging/prod mutation NO during planning. Staging PASS, G47 PASS, G48/G53 planning PASS do not authorize production deploy. |
| Next production gate | â€” | **G49** production dry-run/read-only proof (not executed; owner approval required) |
| G54 HSTS/proxy | â€” | **PASS** â€” HSTS present on staging and production |
...
| # | Gate | Current status |
|---|------|--------|
| 1 | Blocks 1â€“4 + audit remediation complete; CI green | âœ“ PASS â€” SHA `e54445f` or later; CI green on `main` |
| 2 | **Claude full-code audit remediation** | âœ“ PASS â€” Commits `2437c84`â€“`e54445f` on `main`; 5D-B local closeout PASS |
| 3 | Validate PASS | âœ“ PASS â€” `npm.cmd run validate` PASS before Phase C artifact creation |
| 4 | Required local smokes PASS | âœ“ PASS â€” Block A core smokes PASS (5D-B manual fallback); Block 1â€“2 smokes PASS |
| 5 | Working tree clean | âœ“ PASS â€” No uncommitted runtime changes |
| 6 | `main` synced | âœ“ PASS â€” `main` = `origin/main` |
| 7 | No live calls | âœ“ PASS â€” No publish, sync, crawl, or live provider during gate |
| 8 | Phase C refresh proof | âœ“ PASS â€” Controlled refresh complete on `5e1ea5a`; local validation before artifact; staging API recreated; MVP smoke PASS; production untouched (see Â§2.8) |
| 9 | Latest local re-check | âœ“ PASS â€” G43 validate + four focused local smokes PASS on `main` at `a18dcc1`; no repo edits, no commit/push/deploy/staging/VPS/prod (see Â§2.9) |
| 10 | Owner approval for future work | **Requires new explicit approval** â€” Phase C refresh and G43 local re-check are complete; any further staging action requires fresh owner instruction |
...
| Item | Status |
|------|--------|
| Staging deploy proof | **Phase C refresh COMPLETE** â€” G35 Phase C controlled refresh on `5e1ea5a` PASS; staging artifact, API, web, and MVP smoke verified; production untouched (see Â§2.2, Â§2.8); G43 local re-check PASS does not change deferred status; further staging refresh/execution requires fresh explicit owner approval |
| Production deploy proof | Deferred â€” frozen; G48/G53 planning PASS recorded; production deploy ready **NO**; G49/G50 **not executed**; G54 HSTS/proxy fixed; next production path remains G49 dry-run before G50 |
| Puriva Launch | **Blocked** â€” staging/production live proof gates (R2, live AI re-proof, image gen, in-system + email notifications) and product workflow gates required; local live AI + COMPLETED ledger row proven (G77b) and G79 local monthly aggregation do not close launch; **live GA4/GSC WITHDRAWN**; see deferred-scope register |
| Live AI provider / OpenRouter execution | **COMPLETE (local only)** â€” formal clean proof G71e + G71e-retry; G77b persistent COMPLETED ledger row; G79 monthly aggregation includes live `COMPLETED` rows locally; `actualCostUsd=null` remains policy-correct until trusted provider cost ingestion exists; staging/production live proof still pending |
| Live WordPress publish | Deferred â€” draft prep only |
| Live R2 real-bucket proof | Deferred â€” explicit env approval required; no bucket IO in local closeout; no staging/prod storage readiness claim |
| GA/GSC live sync | **WITHDRAWN** â€” MANUAL/snapshot monthly reports remain |
| Scraping / crawling ingestion | Deferred |
| Autonomous agents / background queues | Deferred |
| Client-facing curated MI view | Deferred |
| Revenue Hub | Deferred â€” [`REVENUE_HUB_AI_RH0_OPERATING_MODEL.md`](./architecture/REVENUE_HUB_AI_RH0_OPERATING_MODEL.md) |
| POD AI Toolkit | Deferred â€” [`POD_AI_TOOLKIT_POD0_OPERATING_MODEL.md`](./architecture/POD_AI_TOOLKIT_POD0_OPERATING_MODEL.md) |
...
| `ClientMonthlyBrief` deprecation | Deferred â€” legacy intake active |
...
| `#/monthly-reports` | Client | Alias â†’ same as `#/client-portal` |
...
| Dashboard â†’ Operational readiness | Admin | Block 2 summary |
...
- Market Intelligence Mega Blocks 1â€“3; AI Delivery Revenue Engine Layer 1; Delivery Handoff Layer 2.
- Production Readiness closeout pack; Client Approval happy-path hardening.
- Blocks 1â€“3 (external integrations readiness, admin operations, UI density) on `main`.
- **Block 5Aâ€“5D-A (Claude audit remediation):** closed on `main` â€” admin tenant RBAC, fail-closed boundary smokes, CI unit tests, cross-platform globs, remote staging opt-in, bootstrap guards (`2437c84`â€“`e54445f`).
...
- AI SEO Blocks 3Bâ€“3G, 4Aâ€“4G, 5A, 6Aâ€“6C-v1; Knowledge **local** integration smoke-proven via `smoke:ai-knowledge-context` (not staging/production or live-provider proof).
...
AI SEO admin-operated MVP shell is in place and locally hardened for verified intake, approved KB/context, content-objective flow, compliance checkpoints, and draft-only handoff. Live crawling, Google OAuth / GSC sync, autonomous SEO agents, and production deploy remain deferred. No environment proof has run. See Â§9 and [`deferred-scope-register.md`](./operator/deferred-scope-register.md).
...
Local Puriva E2E operator dry-run proof is now documented in commit `a380bb2` (`test(ops): add Puriva local E2E dry-run proof`). The local operator path is proven end to end: Puriva intake/compliance â†’ AI Knowledge/context â†’ WorkflowBriefs â†’ SEO plan â†’ content/compliance â†’ image/asset handoff â†’ WordPress prepared draft â†’ client-safe monthly report/archive â†’ client approval happy path. Browser smoke labels were aligned to the real UI (`Pending Reviews`, `Request Changes`, `Approve for publication`). Production, staging, VPS, backend/API/schema/auth, and live provider / WordPress / GA/GSC / R2 paths were untouched.
...
| Item | Result |
|------|--------|
| Repo clean at session start | **YES** â€” `git status --porcelain` empty; HEAD at `f6e545b` |
| G49 public read-only probes | **PASS** â€” all 4 probes 200, HSTS present, DB ready (see `G49_PRODUCTION_DRY_RUN_READ_ONLY_PROOF.md` Â§1.1) |
| G49 full gate closure | **NOT executed** â€” owner approval sentence still required |
| Production readiness | Unchanged â€” **NO** |
| RBAC/tenant boundary audit | No BLOCKER found; 1 HIGH (convention-based client-boundary enforcement â€” see full findings) |
...
| Puriva Launch verdict | **BLOCKED** â€” 8 of 15 evaluated areas fully blocked; see `PURIVA_LAUNCH_GATE.md` |
...
| Docs updated | `G49_PRODUCTION_DRY_RUN_READ_ONLY_PROOF.md`, `STAGING_READINESS.md` (Â§12 smoke/gate matrix), `STATUS.md` (this section) |
...
**Result:** Fresh public read-only probe evidence re-collected. **G49 formal gate closure remains NOT complete** â€” the owner-approval sentence required by Â§10 item 1 of the G49 runbook has still not been separately recorded for this exact execution block. **G50 remains NOT authorized/executed. Production readiness remains NO.**
...
| Verdict | **PASS** (Â§6.2 public probe evidence only) |
| G49 full gate closure (owner approval sentence recorded) | **STILL PENDING** |
| G50 production deploy | **NOT EXECUTED / NOT AUTHORIZED** |
| Production readiness | **NO** |
| Mutations performed | **NONE** â€” read-only probes only |
| Log | `$env:TEMP\dca-subagent-b-g49.log` |

Full detail recorded in [`G49_PRODUCTION_DRY_RUN_READ_ONLY_PROOF.md`](./runbooks/G49_PRODUCTION_DRY_RUN_READ_ONLY_PROOF.md) Â§1.2. This entry does not change production readiness status and does not authorize G50.
...
| Admin shell | PASS | â€” | Frozen |
| Client Portal | PASS (final-only) | Proven | Blocked (live proof) |
| WorkflowBriefs | PASS | â€” | Frozen |
| AI Delivery | PASS (review-ready rules) | â€” | Local live proof complete (G77b); staging/prod blocked |
...
| Monthly Reports | PASS (snapshot-first) | â€” | GA/GSC live deferred |
...
| Finance Lite | PASS (boundary docs) | â€” | Payments deferred |
| Revenue Hub | Existing baseline | â€” | Frozen |
| POD Toolkit | Existing baseline | â€” | Frozen |
| Staging | Proven (G47) | PASS | N/A |
| Production readiness | **NO** | â€” | Deploy frozen (G50) |
| Puriva Operating Pack | Pre-live table updated | â€” | Launch blocked |
...
| Item | State |
|------|--------|
| Gate | G69 â€” merge G57â€“G68 to `main` |
| Method | Fast-forward |
| Final `main` commit | `64bfd06` â€” `prelive: complete post-G56 orchestration readiness` |
...
| Live integrations | **None** â€” production frozen |
| Next gate | **G71g** â€” commit/push G71f docs; then G72 model routing policy or G49 formal closure |

## G71f formal clean OpenRouter live proof closeout (2026-07-09)

| Item | State |
|------|--------|
| Gate | G71f â€” docs-only closeout (proof executed across G71e + G71e-retry) |
| `main` commit (docs base) | `953b0c0` â€” docs: record G71 partial OpenRouter proof |
| Operator | Piotr Pakula |
| Formal clean proof | **COMPLETE (local only)** â€” Â§9.14 three-phase sequence validated |
| Phase 1 (G71e) | **PASS** â€” baseline guarded smoke 12/12; `textGateway=local`; `liveProviderCalled=false` |
| Phase 2 (G71e-retry) | **PASS** â€” strict live smoke 12/12; exactly **one** live OpenRouter call |
| Phase 3 restore (G71e-retry) | **PASS** â€” baseline guarded smoke 12/12; local gateway restored |
...
Detail: [`AI_PROVIDER_LIVE_PROOF.md`](./runbooks/AI_PROVIDER_LIVE_PROOF.md) Â§9.15 Â· [`deferred-scope-register.md`](./operator/deferred-scope-register.md).

## G71b / G71c live OpenRouter proof closeout (2026-07-09)

| Item | State |
|------|--------|
| Gate | G71b retry â€” procedural **STOP**; G71c â€” docs-only closeout |
| `main` commit | `6a1c569` â€” docs: record G70 live proof checklist |
| Operator | Piotr Pakula |
| Provider / model | OpenRouter text â€” `anthropic/claude-haiku-4.5` (approved model observed) |
...
| Formal `SMOKE_EXPECT_OPENROUTER_LIVE=true` pass | **Not run** â€” stopped after baseline smoke failure (10/12) |
...
| Local restore | **DONE** â€” `AI_TEXT_GATEWAY=local`; OpenRouter env cleared; API restarted local deterministic; health PASS; restore log `$env:TEMP\dca-g71b-restore-local-gateway.log` |
| Forbidden integrations | **None triggered** â€” no image, email, R2, WordPress, GA/GSC, payment, staging/VPS/production, deploy |
| Secrets | **Not exposed** in proof logs |
| Interpretation | **Partial live proof** â€” not a clean KEEP (procedure failed); not a provider failure |
| Production readiness | **NO** â€” production remains frozen |
| Next gate | **G71e** (optional owner gate) â€” one formal clean live proof using corrected sequence Â§9.14; or proceed to G49 / other live proof gates |

Detail: [`AI_PROVIDER_LIVE_PROOF.md`](./runbooks/AI_PROVIDER_LIVE_PROOF.md) Â§9.13â€“Â§9.14 Â· [`deferred-scope-register.md`](./operator/deferred-scope-register.md).

## Mega-block blocks 1â€“9 coordination closeout (2026-07-09)
...
| 6 WordPress proof prep | **DOCS PASS** | Live draft proof plan Â§6 added; schema gaps documented |
