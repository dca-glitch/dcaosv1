...
> **LOCAL API/AUTH DAST (2026-07-13):** Earlier API/auth dynamic testing was blocked by unprepared local PostgreSQL (`127.0.0.1:5434`). That blocker is resolved. Local Postgres + API health ready; `smoke:local`, `smoke:mvp:local`, client-role boundary, tenant-module, and API integration auth tests passed. Confirmed repairs: malformed JSON → `400 INVALID_JSON` without stack/path leakage; password change/admin reset revoke all active sessions. Staging proof **not required** for this local-first hardening. Production was not tested or mutated.
