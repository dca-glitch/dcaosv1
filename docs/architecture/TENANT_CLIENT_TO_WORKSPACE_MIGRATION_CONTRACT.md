...
The owner authorizes a P2-A implementation-ready offline validator/consumer foundation with these boundaries: (1) the owner prepares the anonymized offline snapshot outside agents; the implementation may validate/consume an owner-provided file only in a later authorized run and never connects to a database; (2) the owner designates exactly one active Tenant before snapshot creation, represented only by a pseudonymous selection label and deterministic hash with no source IDs, names, PII, credentials, or raw records; (3) snapshot/evidence remain outside Git at the single canonical directory `C:\dcaosv1-p2-evidence` (WSL `/mnt/c/dcaosv1-p2-evidence`, same physical location), with no cloud sync or automatic deletion, and the owner decides later deletion after Phase 2 closeout; (4) completeness is fail-closed, requiring deterministic manifest/hash and complete mappings, with every unexpected absence, collision, orphan, cross-tenant link, or unknown role stopping the package; the six known no-role memberships are expected `OWNER_REMEDIATION_REQUIRED` and new exceptions require a new owner decision; (5) `ClientUserAccess` remains unchanged and is the sole per-Client visibility authority, with count/hash preserved and Workspace membership never widening access; (6) the P2-B owner execution gate is `DOCS_ONLY_AUTHORIZED` / `EXECUTION_NOT_AUTHORIZED` ([`../implementation/P2_B_OWNER_EXECUTION_GATE.md`](../implementation/P2_B_OWNER_EXECUTION_GATE.md)); any future P2-C write after a filled single-use P2-B gate is limited to localhost source `127.0.0.1:5434` and isolated restore `127.0.0.1:5435`, with fresh backup/hash and successful restore rehearsal before a separately authorized write, and owner-only resume/rollback; remote/staging/production/VPS remain forbidden; (7) P2-D reconciliation never starts Phase 3, flags remain OFF, endpoint authority remains `LOCAL_ONLY`, and Tenant/Client/ClientUserAccess remain runtime authority until separate owner approval. This authorizes synthetic-fixture implementation and tests only in the P2-A foundation writeback; no snapshot is created, requested, processed, or consumed.

## P2-A owner-run local exporter build boundary

The owner authorizes the build, synthetic/mocked testing, documentation, review, CI, and merge of a separate owner-run local exporter. This is a build-only decision, not authority to execute it or create a real snapshot. The existing P2-A validator remains database-free and offline-only; the exporter must be a separate disabled-by-default module.

If a future owner grants a new explicit single-use authorization, the exporter may use read-only database semantics only against the exact local source target `127.0.0.1:5434`. It must reject a missing target and every other host, port, URL, remote, staging, production, VPS, Tellanic, write/apply/mutation mode, or access-widening attempt. The authorization, connection material, and source data must never be printed, committed, or inspected during build work.

On an authorized future execution, the only permitted output is an already anonymized `DCA_OS_V2_P2_A_SNAPSHOT_V1` file under `C:\dcaosv1-p2-evidence` (WSL `/mnt/c/dcaosv1-p2-evidence`). It must contain only the approved pseudonymous schema, deterministic keys/hashes/mappings/access invariant, and the six expected `OWNER_REMEDIATION_REQUIRED` exceptions; it must exclude prohibited fields, PII, source IDs, credentials, and raw records. This boundary does not authorize database connection during implementation, Docker, migrations, schema changes, backfill, reconciliation, runtime/flag/endpoint changes, dependencies, production/VPS/staging work, or Phase 3.
