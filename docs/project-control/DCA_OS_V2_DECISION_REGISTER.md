...
| V2-015 (P2-A) | Owner prepares the snapshot outside Codex as an anonymized offline file. | DECIDED | The implementation may validate/consume an owner-provided file only in a later authorized run and never connects to a database; this writeback consumes no snapshot and uses synthetic fixtures only. |
...
| V2-017 (P2-A) | Snapshot and evidence retention is external to Git. | DECIDED | Store only at `C:\dcaosv1-p2-evidence`, with no cloud sync or automatic deletion; the owner decides any later deletion after Phase 2 closeout. |
...
| V2-020 (P2-A/P2-B/C posture) | Future preparation is restricted to localhost source and isolated restore. | DECIDED | Use only `127.0.0.1:5434` and `127.0.0.1:5435`, with fresh backup/hash and successful restore rehearsal before any write; Codex aborts and preserves evidence on failure; the owner alone decides resume or rollback. Remote, staging, production, and VPS remain forbidden. |
