...
| Phase 2 owner decisions | IN_PROGRESS | P2-01 approves only a future P2-A population definition: one existing active Tenant from local `127.0.0.1:5434` and all of its active Clients, active TenantMemberships, and active ClientUserAccess records; P2-A must use an offline sanitized snapshot with deterministic manifest/hash and no database connection or mutation. P2-02 no-role disposition remains pending; Phase 2 is NOT_STARTED |
