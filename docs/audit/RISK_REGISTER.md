...
| Risk ID | Title | Area | Current status | Evidence / current control | Gap | Impact | Likelihood | Recommended remediation | Owner | Priority | VPS blocker | Client blocker |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| R-005 | Staging smoke path exercised | QA | Implemented | Controlled staging deployment on commit `80569c68f94481b33dd0a3c2a5a3ec17b41e31cd` executed loopback/public API 200, web/asset 200, DB ready, and auth smoke PASS; remote guard still refuses unapproved hosts | Further staging refreshes still require fresh owner approval; broader browser QA remains separate | Regressions on a future approved staging refresh | Medium | Keep the guarded staging smoke path in runbooks and re-run only on approved targets | Engineering | P1 | Yes | Yes |
...
| R-022 | Production non-root parity not proven | Deployment | Partial | Local candidate image/runtime proof PASS; controlled staging deployment on commit `80569c68f94481b33dd0a3c2a5a3ec17b41e31cd` proved `Config.User=node`, UID/GID `1000/1000`, no permission errors/restarts, and staging API/web/static/auth smoke PASS | Production runtime remains unproven; production deploy not authorized | Wrong runtime user or permission regressions on production | High | Obtain separate owner-approved production deploy/proof before closing globally | Engineering/Ops | P1 | Yes | Yes |
