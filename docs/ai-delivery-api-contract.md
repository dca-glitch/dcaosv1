...
Client Portal monthly reports are implemented as a read-only final client view for linked client users. List: `GET /api/v1/client-portal/projects/:projectId/monthly-reports`. Detail: `GET /api/v1/client-portal/projects/:projectId/monthly-reports/:reportId` returns the final client view with `monthlyReport`, `workSummary`, and `performanceSummary` when an approved metrics snapshot exists. Access requires an authenticated client portal session, an active tenant, `ClientUserAccess`, and a project that belongs to the accessible client. Responses exclude `storageKey`, `adminSummaryNotes`, `tenantId`, `miHandoffId`, `miContextDraft`, workflow internals, raw snapshot records, snapshot import/approve notes, and other admin-only fields. **`performanceSummary` may intentionally include client-safe provenance fields** (`sourceType`, `manualSource`, `placeholderOnly`, `disclaimer`, `itemCount`, normalized GSC/GA4 totals) derived from an approved snapshot — not raw provider payloads.

**Legacy client brief intake (separate from WorkflowBriefs):** `GET/POST/PATCH /api/v1/briefs` and `#/briefs` / `#/client-portal/briefs` serve active **legacy `ClientMonthlyBrief`** monthly/additional content-count intake (`BriefPage`). This is distinct from WorkflowBriefs / Production Plan Review (`/api/v1/workflow-briefs`, `#/workflow-briefs`). Do not remove without a separate approved deprecation block.
...
Client portal report detail (`performanceSummary`, when present): snapshot-first metrics from an **approved** snapshot only. High-level shape may include `targetMonth`, `sourceType`, `manualSource`, `placeholderOnly`, `disclaimer`, `itemCount`, and normalized GSC/GA4 totals (`gscClicks`, `gscImpressions`, `gscAverageCtr`, `gscAveragePosition`, `ga4Sessions`, `ga4Users`, `ga4PageViews`). Does **not** include raw snapshot objects, snapshot `notes`, import/approve metadata, `itemMetrics`, seed markers, or admin summary fields. Live GA/GSC provider sync remains deferred; manual/imported placeholder metrics may show zero totals with a disclaimer.

Client portal UI: canonical live monthly reports are rendered in `#/client-portal` (`ClientPortalPage`). `#/monthly-reports` is currently a stub/static page and does not load this API data.
