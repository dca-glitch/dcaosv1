...
**Current baseline (2026-07-13):** Repo HEAD / Modal docs-closeout `67d9aa4`; Modal impl `a447b9e`. Staging **web** last proven on artifact `/opt/dca/staging-artifacts/a447b9e` (web-only + Modal browser proof). Staging **API** lineage includes later proofs through image-approve `57f1881` and earlier `a8a74e6` / WordPress `bd649d5` — do not collapse API and web SHAs. Host web target `/opt/dca/apps/dcaosv1/staging/web/dist`. **Preferred web deploy:** in-place sync into existing mounted `dist` (preserve inode). Modal Wave historically required shared Caddy remount after directory replacement — classify as `SHARED_PROXY_RESTART`, not production app/API/DB mutation. Production FROZEN. GA/GSC / notification E2E remain deferred non-blocking. **Further staging deploy requires this RC convergence push + controlled artifact.**
