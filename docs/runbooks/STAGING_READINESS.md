...
**Current baseline (2026-07-12):** controlled staging deploy of commit `a8a74e6` PASS; current artifact/API context `/opt/dca/staging-artifacts/a8a74e6`; host-side web target `/opt/dca/apps/dcaosv1/staging/web/dist` (index hash `9cc80489…`); API image `staging-dcaosv1-staging-api:a8a74e6`; no new migrations (`1b8d00d..a8a74e6` schema delta none; migrate status up to date); rollback target retained as `1b8d00d` (image tag + compose context + web backup `dist-before-a8a74e6-20260712-042923`); rollback rehearsal PASS; public frontend HTTP 200; public and loopback (`127.0.0.1:4011`) API health PASS; `dcaosv1-staging-postgres` healthy (StartedAt unchanged); Caddy not restarted or changed; production untouched/FROZEN; live integrations remain unproven; **further staging live-proof (Workstream 7) or any production deploy requires fresh explicit owner approval**.

**Ground-truth notice (updated 2026-07-12 post-`a8a74e6` deploy):** Controlled staging deploy of commit `a8a74e6` is PASS. Current staging artifact/API context is `/opt/dca/staging-artifacts/a8a74e6`; host-side web target is `/opt/dca/apps/dcaosv1/staging/web/dist`; staging compose is `/opt/dca/apps/dcaosv1/staging/docker-compose.staging.yml`; compose requires `--env-file .env.staging`; correct staging API compose service is `dcaosv1-staging-api`, not `api`. No `prisma migrate deploy` performed (schema already up to date). Final proof: public frontend HTTP 200, public API health PASS, loopback API health on `127.0.0.1:4011` PASS, MVP staging smoke PASS, staging security baseline 32/32 PASS. Caddy was not restarted or changed. Production deploy attempted: NO. Production app/API/DB mutation: NO. Prior `1b8d00d` deploy remains the retained rollback baseline. Any further staging live-proof/VPS execution/migration/deploy requires fresh explicit owner approval with bounded execution block instructions. This docs update authorizes no new VPS, staging, production, deploy, DNS, migration, SSH, Docker, or Caddy action without explicit owner instruction.
...
Controlled staging deploy `a8a74e6` is **COMPLETE**. Local Workstreams 1–5 and Workstream 6 are closed for this cycle. Before requesting Workstream 7 live proofs:

| Item | State |
|------|--------|
| Local Workstreams 1–5 | **COMPLETE** at `a8a74e6` (pre-staging convergence) |
| Staging Workstream 6 | **COMPLETE** — artifact `/opt/dca/staging-artifacts/a8a74e6`; health PASS; MVP + security smokes PASS |
| Vite high dependency finding | **COMPLETE** — Vite `6.4.3`; high finding closed; commit `95af080`; validate PASS |
| Canonical component-system decision | **COMPLETE** — public `apps/web/src/components/ui`; private `apps/web/src/design-system`; import guard baseline 27 frozen / 0 new at deploy |
| Rollback / compatibility plan | **COMPLETE** — target `1b8d00d` retained; schema delta none |
| Rollback rehearsal | **PASS** (2026-07-12) |
| SHA-tagged retained staging API image | **PASS** — `:a8a74e6` current; `:1b8d00d` retained |
...
| Staging deploy | **Controlled deploy of `a8a74e6` COMPLETE** — artifact `/opt/dca/staging-artifacts/a8a74e6`; API image `:a8a74e6`; public frontend HTTP 200; public and loopback API health PASS; no new migration; rollback target `1b8d00d` retained; Caddy unchanged; production untouched; prior `1b8d00d` deploy is historical/rollback baseline; further Workstream 7 live proofs require fresh owner approval |
...
- **Controlled staging deploy of `1b8d00d` (2026-07-11):** PASS. Local `npm.cmd run validate` PASS before artifact creation. Staging artifact created and uploaded to `/opt/dca/staging-artifacts/1b8d00d`. Staging DB backed up to `/opt/dca/apps/dcaosv1/staging/backups/staging-db-before-1b8d00d-20260711-200155.dump`. Applied migrations `20260709120000_add_ai_budget_ledger` and `20260711115000_add_in_app_notifications`. Staging compose API context updated to `/opt/dca/staging-artifacts/1b8d00d`. Staging API recreated successfully. Public frontend HTTP 200; public API health PASS; loopback API health on `127.0.0.1:4011` PASS; `dcaosv1-staging-postgres` healthy and accepting connections. Web dist deployed successfully. Caddy not restarted or changed. Production untouched; production health HTTP 200 during deploy. Post-deploy smoke PASS. No commit/push/deploy in repo.
