...
**Current baseline (2026-07-11):** latest proven full local closeout commit `217c11c` (`test: stabilize G35 Phase B browser smokes`); controlled staging deploy of commit `1b8d00d` completed 2026-07-11 with post-deploy smoke PASS; current artifact/API context `/opt/dca/staging-artifacts/1b8d00d`; host-side web target `/opt/dca/apps/dcaosv1/staging/web/dist`; applied migrations `20260709120000_add_ai_budget_ledger` and `20260711115000_add_in_app_notifications`; staging DB backup at `/opt/dca/apps/dcaosv1/staging/backups/staging-db-before-1b8d00d-20260711-200155.dump`; public frontend HTTP 200; public and loopback (`127.0.0.1:4011`) API health PASS; `dcaosv1-staging-postgres` healthy and accepting connections; Caddy not restarted or changed; production untouched; prior G35 Phase C refresh on `5e1ea5a` and G46d/G47/G47b/G47c PASS remain historical context only; **further staging refresh/execution or any production deploy requires fresh explicit owner approval**.

**Ground-truth notice (updated 2026-07-11 post-`1b8d00d` deploy):** Controlled staging deploy of commit `1b8d00d` is PASS. Current staging artifact/API context is `/opt/dca/staging-artifacts/1b8d00d`; host-side web target is `/opt/dca/apps/dcaosv1/staging/web/dist`; staging compose is `/opt/dca/apps/dcaosv1/staging/docker-compose.staging.yml`; compose requires `--env-file .env.staging`; correct staging API compose service is `dcaosv1-staging-api`, not `api`. Applied migrations: `20260709120000_add_ai_budget_ledger`, `20260711115000_add_in_app_notifications`. Final proof: public frontend HTTP 200, public API health PASS, loopback API health on `127.0.0.1:4011` PASS, `dcaosv1-staging-postgres` healthy and accepting connections. Caddy was not restarted or changed. Production deploy attempted: NO. Production app/API/DB mutation: NO. Any further staging refresh/VPS execution/migration/deploy requires fresh explicit owner approval with bounded execution block instructions. This docs update authorizes no new VPS, staging, production, deploy, DNS, migration, SSH, Docker, or Caddy action without explicit owner instruction.
...
| Staging deploy | **Controlled deploy of `1b8d00d` COMPLETE** — artifact `/opt/dca/staging-artifacts/1b8d00d`; API recreated; public frontend HTTP 200; public and loopback API health PASS; migrations `20260709120000_add_ai_budget_ledger` and `20260711115000_add_in_app_notifications` applied; staging DB backup at `/opt/dca/apps/dcaosv1/staging/backups/staging-db-before-1b8d00d-20260711-200155.dump`; Caddy unchanged; production untouched; prior G35 Phase C refresh on `5e1ea5a` is historical (see STAGING_READINESS §2.1 and STATUS §2.2/§2.8); further staging work requires fresh owner approval |
