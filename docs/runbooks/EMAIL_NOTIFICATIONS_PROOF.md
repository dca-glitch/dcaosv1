...
**Status:** Docs-only audit + proof-preparation deliverable. No live email was sent, no `RESEND_API_KEY` was set or read for value, and no network call was made to produce this document. Local disabled-safe behavior was confirmed by code inspection and one isolated, no-network unit test run.
...
|---|---|---|---|
| 1 | **Article ready** | `AI_DELIVERY_DELIVERABLE_READY` (`markAiDeliveryDeliverableReady` in `core.runtime.ts`) | **Internal-only.** Fires `recordAiDeliverySystemEvent`; always `SKIPPED`; recipient is the internal placeholder only. No real admin/client mailbox is notified when a deliverable is marked ready. |
| 2 | **Image set ready** | `AI_DELIVERY_ARTICLE_IMAGE_FINAL_READY` (article image → `FINAL_READY` status transition in `core.runtime.ts`) | **Internal-only.** Same gap as #1 — no real mailbox notified. |
...
| 5 | **Admin action required** | No single event — closest matches are: brief `SUBMITTED` (real path, `admin@dca.local`); client rejection (#4, real path, is itself an admin-action-required signal); and the three "review requested" internal events (`AI_DELIVERY_CONTENT_DRAFT_REVIEW_REQUESTED`, `AI_DELIVERY_CONTENT_PLAN_REVIEW_REQUESTED`, `AI_DELIVERY_BRIEF_CLIENT_INPUT_REQUESTED`) | **Partially wired.** Brief-submitted and client-rejection reach real inboxes. The three "review requested" events reach only the internal-only path — an admin is not actually emailed when a content draft or content plan is marked ready for review, only an `EmailLog` row with the internal placeholder recipient is written. |
| 6 | **Monthly report final** | No event constant exists. Searched `monthly-report-pdf.service.ts`, `puriva-monthly-report.ts`, `finance-monthly-report-pdf.service.ts` for any `sendEmailNotification` / `recordAiDeliverySystemEvent` / `EmailLog` reference | **Not wired at all.** Neither pathway fires when a monthly report transitions to `FINAL`. No `EmailLog` row of any kind is produced by this transition today. |
| 7 | **WordPress draft prepared** | No event constant exists. Searched `client-publication.runtime.ts`, `workflow-brief-publication-handoff.execution.ts`, `wordpress.service.ts`, `workflow-brief.runtime.ts` for any `sendEmailNotification` / `recordAiDeliverySystemEvent` / `EmailLog` reference | **Not wired at all.** `prepare-wordpress-draft` (see [`WORDPRESS_DRAFT_PROOF.md`](./WORDPRESS_DRAFT_PROOF.md) §2.1) produces no notification of any kind — this is a genuine gap distinct from and in addition to the "no auto-publish" behavior, which is correct and intentional. The *absence of a notification* is not a safety feature here; it is an unimplemented event.
