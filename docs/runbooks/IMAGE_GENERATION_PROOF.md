...
**Status:** Spec + proof plan. **OpenAI Images staging one-image AI Policy path = STAGING LIVE PROVEN (2026-07-12)** — one bounded neutral image only. Three-image sets, regeneration, client approval, WordPress, and production image generation remain **NOT PROVEN**. BFL successful generation remains **NOT PROVEN**.

**Related:** [`../architecture/AI_MODEL_POLICY.md`](../architecture/AI_MODEL_POLICY.md) **(image provider/model policy — §2; active route = OpenAI `gpt-image-1`)** · [`../architecture/AI_POLICY_PROVIDER_ROUTING.md`](../architecture/AI_POLICY_PROVIDER_ROUTING.md) **(AI Policy / adapter architecture — OpenAI Images via `ImageProviderAdapter`; BFL retained inactive)** · [`PURIVA_LAUNCH_GATE.md`](./PURIVA_LAUNCH_GATE.md) · [`STORAGE_R2_PROOF.md`](./STORAGE_R2_PROOF.md) · [`INTEGRATIONS_TRUTH_MATRIX.md`](./INTEGRATIONS_TRUTH_MATRIX.md) · [`PURIVA_IMAGE_PACKAGE_V1_GATE.md`](./PURIVA_IMAGE_PACKAGE_V1_GATE.md)

**Architecture alignment (2026-07-12):** Canonical path is AI Policy image route → `ImageProviderAdapter` → concrete adapter → one-image guard → normalized result → optional private R2 → ledger. **Staging live proof (OpenAI):** marker `DCA-IMG-OPENAI-20260712T102457Z-a08x91rb` on artifact `c07df10`; provider=`openai` broker=`direct` model=`gpt-image-1` quality=`low` size=`1024x1024`; exactly one request/output; retry=0; fallback=false; PNG 1024×1024; sha256 `5892cadc…a82f`; COMPLETED ledger; R2 persistence not applicable; live flags restored disabled. Workflows must not call vendors directly. Do not create a parallel image-only routing system.
---
