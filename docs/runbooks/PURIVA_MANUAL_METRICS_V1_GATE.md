...
5. **Client boundary** — DRAFT report remains hidden from client portal until promoted to FINAL; FINAL reports and delivery summaries expose sanitized `displayTitle`/`title` (no `[PURIVA_LOCAL_SETUP]`, `PURIVA_*_V1`, or manual-metrics markers), plus `performanceSummary.placeholderOnly`, `manualSource`, and client-safe `disclaimer` (never raw notes, itemMetrics embed, or internal markers)
