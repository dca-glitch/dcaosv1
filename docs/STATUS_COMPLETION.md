...
| **Clients (CRM)** | **88%** | Done, with an open finding | CRUD, filters, `clientKind`, website. No dedicated standalone smoke exists for Clients CRUD; client records are otherwise exercised only as fixtures inside other passing smokes. **G29 finding (2026-07-06):** `smoke:client-domain:browser` reproducibly fails waiting for the "Add Client" modal (`.modal-panel` with text "Add Client") to become visible within 10s after clicking the "Add Client" button — confirmed on two consecutive runs. Not fixed in this closeout (docs/smoke-only scope); needs a Sonnet-level frontend investigation before this row can move past 88%. |
| **Client Hub + domain model (block 1)** | **97%** | Done, with an open finding | Hub UI covers catalog, inquiries shell, and publication log — re-confirmed in this closeout via `smoke:client-hub:catalog-inquiry:browser`, `smoke:client-hub:edge-cases:browser`, and `smoke:client-hub:publication-log:browser` (all PASS, 2026-07-06). **Not bumped to 100%:** the row's other named proof source, `smoke:client-domain:browser`, fails reproducibly (same Add Client modal timeout noted in the Clients (CRM) row above) before it reaches the Client Hub sections of that script. Treat Client Hub catalog/inquiry/publication-log coverage as proven; treat the broader client-domain regression script as broken pending investigation. |
...
| **Projects & Tasks** | **88%** | Done, coverage gap noted | Admin MVP closed per `docs/operator/module-completion-matrix.md` ("Closed for MVP"). No dedicated standalone smoke script exists for Projects & Tasks; the CRUD surface is exercised only as an incidental fixture inside other passing smokes (e.g. AI Delivery project creation). Not bumped — no direct proof to point to. |
...
| Block | Name | % | Status |
|-------|------|---|--------|
| 1 | Client foundation + `clientKind` | **92%** | Done, with an open finding — see Clients (CRM) row above (`smoke:client-domain:browser` Add Client modal failure) |
...
Blocks 2, 3, 4, and 6 now carry honest local-only labels backed by fresh 2026-07-06 smoke proof (see area rows above for exact PASS counts). Block 1 (client foundation / Clients CRM) stays at 92% pending the open Add Client modal regression finding. Block 5 was already 100% local-safe. A single blended "average" is no longer meaningful across mixed percentage/label rows and has been removed.
