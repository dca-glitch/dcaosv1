...
import { trashWordPressDraftByExactId } from "./wordpress-live-draft.adapter";
import { createFakeWordPressTransport } from "./wordpress-live-draft.fake-transport";
import type { BoundedProofCleanupProviders } from "./ai-delivery-bounded-workflow.live-providers";
...
export function createBoundedFakeWorkflowProviders(options: BoundedFakeProviderOptions = {}): {
  ...
  const providers: BoundedWorkflowProviders = {
  ...
  const cleanupProviders: BoundedProofCleanupProviders = {
    async deletePrivateImage(input) {
      if (!stored.has(input.storageKey)) {
        return { ok: false };
      }
      stored.delete(input.storageKey);
      stats.storageDeletes += 1;
      return { ok: true };
    },
    async trashWordPressDraft(input) {
      ...
      stats.wordpressTrashes = wordpress.stats.trashCount;
      return { ok: result.ok };
    }
  };
