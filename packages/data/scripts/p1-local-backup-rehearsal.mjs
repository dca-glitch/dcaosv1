...
import { assertLocalTarget, executeBackfill, reconcile } from "./p1-execution-local.mjs";
...
export function parseBackupArgs(args) {
  ...
  if (!parsed.backupDir || !path.isAbsolute(parsed.backupDir)) throw new Error("--backup-dir must be an absolute path outside the repository.");
  return parsed;
}
...
export async function runBackupRehearsal(args, env = process.env, io = { stdout: process.stdout, stderr: process.stderr }) {
  try {
    ...
    docker(["exec", "-i", RESTORE_CONTAINER, "pg_restore", "-C", "-d", "postgres", "-U", "postgres"], { input: dump, encoding: "buffer", maxBuffer: 1024 * 1024 * 1024 });
